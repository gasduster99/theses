...
B0 = 10000
par    = c(r0, K0, s0)#, B0)
parLow = c(.Machine$double.eps, .Machine$double.eps, .Machine$double.eps)#, 10^-20)
parHi  = c(1e0, 1e5, 1e0)#, 10000)
optimOut = optim(par, ll, x=y,	
	...
	control = list(
		maxit=1e9,
		#parscale=c(0.001, 0.1, 1000, 0.1),
		ndeps=c(.Machine$double.eps, .Machine$double.eps, .Machine$double.eps),#, 0.1),
		pgtol=.Machine$double.eps,
		factr=.Machine$double.eps
	)
)
#output
sHat  = optimOut$par[3]	#0.12    #
#qHat  = optimOut$par[1]	#0.00045 #
rHat  = optimOut$par[1]	#0.39    #
KHat  = optimOut$par[2]	#2709    #
