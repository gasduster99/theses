...
\begin{minipage}{0.7\textwidth}
	...
	%\hspace*{1cm}
	\begin{center}
	{\huge \textbf{A Metamodel Based Clustering of Fisheries Reference Point Estimation.}}\\ % in Sparsely Sampled, Mixed Stock Fisheries.}\\
	\vspace{0.5cm}
...
\headerbox{1. Fisheries Model}{name=model,column=0,below=introduction,span=2}{
	...
	%
		...
		\item Most models assume\\ a two parameter form. %\\ Although a three parameter  
		\item $\gamma=-1$ (black curve) is most common.%ly used model.
		%\item Two v. Three Parameter
		%\item Most models assume $\gamma=-1$
		\item Freeing $\gamma$ expands RPs. %Two parameter limits to line
	...
	\begin{itemize}[leftmargin=*]
		...
		\item Inferred by learning $\theta$\\ as possible. %from data.
                \item RPs are the foundation of mangement action.
        \end{itemize}
	$~$\\$~$\\$~$\\
	\end{minipage}
        \begin{minipage}[b]{0.48\textwidth}
	\includegraphics[width=0.8\textwidth]{../../ddBias/rpCurves.png} %designLineColorHHardFlatT30N150WWideN112.png}
	$~$\\$~$\\$~$\\
	\end{minipage}
	$~$\vspace*{-2cm}
	%%
...
\headerbox{2. Simulation \& Metamodeling}{name=sim,column=2,below=introduction,span=1}{
	%
	Datasets are simulated broadly in RP space and fit with misspecified two-parameter $R(B)$. 
	Each dataset produces an example bias mapping. 
	%
	\begin{equation*}
