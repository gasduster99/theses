...
f = Vectorize(getZeta, "gamma")

#
isSE = T
#f = function(x) inv.logit(x) #x^3 #
Xlimits = c(-10, 10) #c(-5, 5)
Ylimits = c(0.1, 0.6) #c(0, 1) #f(Xlimits) #
##
#isSE = F
...
X = seq(Xlimits[1], Xlimits[2], length.out=nStart) #c(Xlimits[1], runif(nStart, Xlimits[1], Xlimits[2]), Xlimits[2])
y = f(X)
#
Xall = t(t(X))
Yall = t(t(y))
...
for(i in 1:M){
	...
	X = maximinFV(1, 1, fHat, Xlimits, Ylimits, TT=10000, XStart=Xall, f_se=isSE)
	y = f(X)
	#add new evaluations to all evaluations
	Xall = rbind(Xall, X)
	Yall = rbind(Yall, y)
