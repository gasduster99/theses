...
logML = function(x, fb){
	#
	l1=x['l1']; l2=x['l2']; th=x['th']
	nu=x['nu']; s2=x['s2']
	#     from like             from beta integral
	out = -0.5*logDet(fb$V) + -0.5*logDet(t(fb$LIX)%*%fb$LIX) + -0.5*fb$S2 + 
		s2Prior(s2) + nuPrior(nu) + l1Prior(l1) + l2Prior(l2) + thPrior(th)
	#
	return(out)
}

#
funnyBusiness = function(l1, l2, th, nu, s2){
	#
	#dis = anisoNorm(X, l1, l2, th)
	SIG = s2*maternCor(anisoNorm(axes, axes, l1, l2, th), nu) + Tg
	#print(c(l1, l2, th, nu, s2))
	#print(SIG)
	#
	L  = t(chol(SIG))
...
#
registerData = function(y, X, axes, Tg, l1a=.GlobalEnv$l1a, l1b=.GlobalEnv$l1b, l2a=.GlobalEnv$l2a, l2b=.GlobalEnv$l2b, nua=.GlobalEnv$nua, nub=.GlobalEnv$nub, tha=.GlobalEnv$tha, thb=.GlobalEnv$thb, s2a=.GlobalEnv$s2a, s2b=.GlobalEnv$s2b){
	...
	.GlobalEnv$nua=nua; .GlobalEnv$nub=nub
	.GlobalEnv$tha=tha; .GlobalEnv$thb=thb
	.GlobalEnv$s2a=s2a; .GlobalEnv$s2b=s2b
}

#c(rep(eps(), 5),0,rep(eps(), 2))
gpMAP = function( par, lower=rep(eps(), 5), upper=c(Inf, Inf, pi/2, 10, Inf), hessian=F, psiSample=F ){	
	#
	realPar = par
	realPar['l1'] = log(realPar['l1'])
	realPar['l2'] = log(realPar['l2'])
	realPar['th'] = logit(realPar['th']/(0.5*pi))
	realPar['nu'] = log(realPar['nu'])
	realPar['s2'] = log(realPar['s2'])
	#
	opt = optim(realPar,
        	function(lp){
			p = c(exp(lp['l1']), exp(lp['l2']), inv.logit(lp['th'])*(0.5*pi), exp(lp['nu']), exp(lp['s2']))
			fb = funnyBusiness(p['l1'], p['l2'], p['th'], p['nu'], p['s2'])
			return(-logML(p, fb))
        	},
        	control=list('maxit'=10^6),
        	hessian=(hessian | psiSample!=F), 
        	method="L-BFGS-B",
        	lower=c(log(lower[1:2]), logit(lower[3]/(0.5*pi)), log(lower[4:5])), 	#c(rep(eps(), 2),0,rep(eps(), 2)),
        	upper=c(log(upper[1:2]), logit(upper[3]/(0.5*pi)), log(upper[4:5]))	#c(1, 1, pi/2, 3, 1)
	)
	#
	outPar = opt$par
	outPar['l1'] = exp(outPar['l1'])
	outPar['l2'] = exp(outPar['l2']) 
       	outPar['th'] = inv.logit(outPar['th'])*(0.5*pi)
        outPar['nu'] = exp(outPar['nu'])
	outPar['s2'] = exp(outPar['s2'])
	#
	fb = funnyBusiness(outPar['l1'], outPar['l2'], outPar['th'], outPar['nu'], outPar['s2'])
	#outPar = c(beta=fb$bHat, outPar)
	...
	if( hessian | psiSample!=F ){ 
		...
		sam[,'l1'] = exp(sam[,'l1'])
        	sam[,'l2'] = exp(sam[,'l2'])
        	sam[,'th'] = inv.logit(sam[,'th'])*(0.5*pi)
        	sam[,'nu'] = exp(sam[,'nu'])
        	sam[,'s2'] = exp(sam[,'s2'])		
		#
		out$betaCov = betaCov
...
#library(stringr)
##
#dat = read.csv('/home/nick/Documents/school/ucscGrad/stat226/hw4/D.csv')[,-1]
#set.seed(2)
...
#
##
#par=c(l1=0.5, l2=0.5, th=eps(), nu=1.5, s2=0.28)
