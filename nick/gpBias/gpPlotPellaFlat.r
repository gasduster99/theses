...
#xi bias
png(sprintf("xBiasPella%s.png", mod))
#
...
image(xiStar*M, zetaStar, xBias*M, #xiStar[xiMask], zetaStar[zetaMask], xBias[xiMask, zetaMask],
	col  = adjustcolor(xCols, alpha.f=0.6),  #hcl.colors(41, "RdBu", rev=T)
        xlab = TeX("$F^*$"),#'Xi',
        ylab = TeX('$B^*/B_0$'),
	main = TeX("Bias in Estimated $F^*$"), #Optimal Fishing",
...
legend(grconvertX(415, "device"), grconvertY(90, "device"), #grconvertX(0.5, "device"), grconvertY(1, "device"),  #
        sprintf("%1.1f", rev(seq(min(xBias[xiMask, zetaMask], na.rm=T)*M, max(xBias[xiMask, zetaMask], na.rm=T)*M, length.out=length(show)))),
...
#xi relative bias
png(sprintf("xRelBiasPella%s.png", mod))
##
#maxXBias = abs(max(xBias/xiStar, na.rm=T))
#minXBias = abs(min(xBias/xiStar, na.rm=T))
#posCols = hcl.colors(round(100*maxXBias/(maxXBias+minXBias)), "Reds 2", rev=T)
#negCols = hcl.colors(round(100*minXBias/(maxXBias+minXBias)), "Blues 2", rev=F)
maxAbsXBias = max(abs(xBias/xiStar), na.rm=T)
posCols = hcl.colors(50, "Reds 2", rev=T)
negCols = hcl.colors(50, "Blues 2", rev=F)
xCols = c(negCols, "#FFFFFF", posCols)
#
xiMask = xiStar>xiBot & xiStar<xiTop
zetaMask = zetaStar>zetaBot & zetaStar<zetaTop
#
par(mar=c(5, 4, 4, 5)+0.1)
image(xiStar*M, zetaStar, xBias/xiStar, #xiStar[xiMask], zetaStar[zetaMask], xBias[xiMask, zetaMask],
	col  = adjustcolor(xCols, alpha.f=0.6),  #hcl.colors(41, "RdBu", rev=T), #
        xlab = TeX("$F^*$"),#'Xi',
        ylab = TeX('$B^*/B_0$'),
	main = TeX("Relative Bias in Estimated $F^*$"), #Optimal Fishing",
	ylim = c(zetaBot, zetaTop),
	xlim = c(xiBot*M, xiTop*M),
