...
M = 0.2
gamma = eps()*10000 #smaller than eps()*10000 gives numerical zig zags
B0=10000
#
xis = seq(0.01,4,0.01)
zetaR = sapply(xis, function(xi){
		ff = xi*M
		zR = PBar(getAlpha(gamma, ff, M), getBeta(getAlpha(gamma, ff, M), gamma, M, B0), gamma, ff, M)/
			PBar(getAlpha(gamma, ff, M), getBeta(getAlpha(gamma, ff, M), gamma, M, B0), gamma, 0, M)
	}
)
zetaRM = sapply(xis, function(xi){
		ff = xi*M
		zR = PBar(getAlpha(-gamma, ff, M), getBeta(getAlpha(-gamma, ff, M), -gamma, M, B0), -gamma, ff, M)/
			PBar(getAlpha(-gamma, ff, M), getBeta(getAlpha(-gamma, ff, M), -gamma, M, B0), -gamma, 0, M)
	}
...
curve(1/(x+2), from=0, to=4, lwd=3)
lines(xis, zetaRM, col='red')
lines(xis, zetaR, col='blue')
