...
#fill in some reasonable values for noted values
model = "test"
depl.level = 1
sigma = 0.1
pre.fishery.yrs = 5
...
##rewrite for unix systems
#dir.create(sprintf("./DBSRA/%s/", LH))
#dir.create(sprintf("./DBSRA/%s/%s/", LH, model)) #NOTE: model not defined
#dir.create(sprintf("./DBSRA/%s/%s/Run_Data/", LH, model))
# 
## wd <- paste(drive,":\\DBSRA\\",LH,"\\",model,sep="")
## setwd(wd)
##rewrite for unix
#wd = sprintf("./DBSRA/%s/%s/", LH, model)
#setwd(wd) 
# 
...
##rewrite for unix
#dynamics = sprintf("%s/Run_Data/results_%s_%s/", wd, depl.level, sigma) #NOTE: depl.level & sigma not defined

#a simplified unix version
place = sprintf("./%s%s%s/", LH, SRR, model)
dir.create(place)
source("./LH_parameter_values.R")
dynamics = sprintf("%s/results.RData", place) #sprintf("%s/Run_Data/results_%s_%s/", wd, depl.level, sigma)

#Read in the Catch History =======================================================================
 #NOTE: pre.fishery.yrs & fishery.yrs not defined
...
#Dump the import data into storage matrices
  ...
  #depl.matrix[,nsim] <- dyn$depl #NOTE: dyn$depl not defined  
  Ry.matrix[,nsim] <- 2*dyn$Ry
  ln.Ry.matrix[,nsim] <- log(2*dyn$Ry)
  #vul.total.matrix[,nsim] <- dyn$vul.total #NOTE: dyn$vul.total not defined
...
#sim_info[[22]] <- obs.data$store.probs #NOTE: obs.data not defined
