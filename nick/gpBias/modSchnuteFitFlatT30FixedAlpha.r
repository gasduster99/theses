...
place = "./modsSchnuteExpT45FixedAlphaFlatT30/" #"./modsSchnuteExpT45FixedBetaFlatT30/" #"./modsSchnuteHHardFlatT30N150WWideN112/" #84/56/28/ #"./modsSchnuteHHardFlatT30N150WWideExtra/" #'./modsSchnuteHHardFlatT30N150WWide/'
odeMethod = "lsode"
...
#foreach(i=floor(length(datFiles)/2):length(datFiles), .options.multicore = opts) %dopar% {
#foreach(i=seq(floor(length(datFiles)/2)-1, 1, -1), .options.multicore = opts) %dopar% {
#for(i in 1:length(datFiles)){
	...
	#skip if the fitFix model has already been fit.
	fileFit = gsub("datGen", "fitFix", datFiles[i])
	if( file.exists(fileFit) ){ next }
	
	#skip if the Exp fit model has not been fit yet.
	if( !file.exists(gsub("datGen", "fit", datFiles[i])) ){ next }
	...
	optAns = fit$optimize(cpue,
	        c('lsdo', 'lbeta'), #, 'lalpha'), # 
	        lower   = c(log(0.001), -10), #, log(M)), #	
	        upper   = c(log(1), -2), #log(getBeta(100, -1, M, P0))  #, log(100)), #
