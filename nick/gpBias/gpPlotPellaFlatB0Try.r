...
D = D[!is.na(D$xiInv),]
#
D$lKV[D$lKV==0] = max(D$lKV[D$lKV!=0])
###
...
lky = D$lK
Tlk = diag(c(D$lKV))
XK = cbind(1, D$xiInv, D$zetaInv)
axes = XK[,2:3]
#
registerData(lky, XK, axes, Tlk)
#registerData(D$lF, X, axes, Tg)
par = c(l1=0.5, l2=0.5, s2=0.5)
gpFitLK = gpMAP(par, hessian=F, psiSample=F, lower=c(eps(), rep(eps(), 2)))
...
#
gpPredLK = gpPredict(XStar, XStar[,2:3], gpFitLK)
gpPredLK[!mask] = NA

##bias
#xiHat = exp(gpPred)/M
#xBias = sweep(xiHat, 1, xiStar)
kBias = exp(gpPredLK)-P0
bMSYBias = sweep(exp(gpPredLK)/2, 2, zetaStar*P0)
...
#        }, xiHat*M, XStar[,2]*M, XStar[,3], mc.cores=6 #detectCores()
#)
...
#
maxKBias = abs(max(kBias, na.rm=T))
minKBias = abs(min(kBias, na.rm=T))
nCol = 25
posCols = hcl.colors(round(nCol*maxKBias/(maxKBias+minKBias)), "Reds 2", rev=T)
negCols = hcl.colors(round(nCol*minKBias/(maxKBias+minKBias)), "Blues 2", rev=F)
...
image(xiStar*M, zetaStar, kBias, #xiStar[xiMask], zetaStar[zetaMask], xBias[xiMask, zetaMask],
	col  = adjustcolor(kCols, alpha.f=0.6),  #hcl.colors(41, "RdBu", rev=T)
        xlab = TeX("$F^*$"),#'Xi',
        ylab = TeX('$B^*/B_0$'),
	main = TeX("Bias in Estimated $K$"), #Optimal Fishing",
...
legend(grconvertX(415, "device"), grconvertY(90, "device"), #grconvertX(580, "device"), grconvertY(150, "device"), #grconvertX(0.5, "device"), grconvertY(1, "device"),  #
        ...
        fill = rev(kCols[show]), #colMap[c(1, 10, 20)], 
        xpd = NA
)
dev.off()
#
...
##
#maxXBias = abs(max(xBias/xiStar, na.rm=T))
#minXBias = abs(min(xBias/xiStar, na.rm=T))
...
posCols = hcl.colors(15, "Reds 2", rev=T)
negCols = hcl.colors(15, "Blues 2", rev=F)
...
#
par(mar=c(5, 4, 4, 5)+0.1)
image(xiStar*M, zetaStar, kBias/P0, #xiStar[xiMask], zetaStar[zetaMask], xBias[xiMask, zetaMask],
	col  = adjustcolor(xCols, alpha.f=0.6),  #hcl.colors(41, "RdBu", rev=T), #
        xlab = TeX("$F^*$"),#'Xi',
        ylab = TeX('$B^*/B_0$'),
	main = TeX("Relative Bias in Estimated $K$"), #Optimal Fishing",
	ylim = c(zetaBot, zetaTop),
	xlim = c(xiBot*M, xiTop*M),
...
legend(grconvertX(415, "device"), grconvertY(90, "device"), #grconvertX(580, "device"), grconvertY(120, "device"), #grconvertX(0.5, "device"), grconvertY(1, "device"),  #
        sprintf("%1.2f", rev(seq(-maxAbsXBias, maxAbsXBias, length.out=length(show)))), #rev(seq(min(xBias[xiMask, zetaMask]/xiStar[xiMask], na.rm=T), max(xBias[xiMask, zetaMask]/xiStar[xiMask], na.rm=T), length.out=length(show)))), #rev(seq(-0.5, 0.5, length.out=length(show)))),#
...
#
par(mar=c(5, 4, 4, 5)+0.1)
image(xiStar*M, zetaStar, bMSYBias, #xiStar[xiMask], zetaStar[zetaMask], xBias[xiMask, zetaMask],
	col  = adjustcolor(bMSYCols, alpha.f=0.6),  #hcl.colors(41, "RdBu", rev=T)
        xlab = TeX("$F^*$"),#'Xi',
        ylab = TeX('$B^*/B_0$'),
	main = TeX("Bias in Estimated $B^*$"), #Optimal Fishing",
	ylim = c(zetaBot, zetaTop),
	xlim = c(xiBot*M, xiTop*M)
	#,zlim = c(min(xBias, na.rm=T), 17)
)
...
legend(grconvertX(415, "device"), grconvertY(90, "device"), #grconvertX(580, "device"), grconvertY(150, "device"), #grconvertX(0.5, "device"), grconvertY(1, "device"),  #
        ...
        fill = rev(bMSYCols[show]), #colMap[c(1, 10, 20)], 
        xpd = NA
)
dev.off()
...
#
par(mar=c(5, 4, 4, 5)+0.1)
image(xiStar*M, zetaStar, t(t(bMSYBias)/(P0*zetaStar)), #xiStar[xiMask], zetaStar[zetaMask], xBias[xiMask, zetaMask],
	col  = adjustcolor(xCols, alpha.f=0.6),  #hcl.colors(41, "RdBu", rev=T), #
        xlab = TeX("$F^*$"),#'Xi',
        ylab = TeX('$B^*/B_0$'),
	main = TeX("Relative Bias in Estimated $B^*$"), #Optimal Fishing",
	ylim = c(zetaBot, zetaTop),
	xlim = c(xiBot*M, xiTop*M)
	,zlim = c(-maxAbsXBias, maxAbsXBias)
	#,zlim = c(min(xBias, na.rm=T), 17)
...
##zeta bias
#png(sprintf("yBiasPella%s.png", mod))
##
...
##
#par(mar=c(5, 4, 4, 5)+0.1)
#image(xiStar*M, zetaStar, yBias,
#	col  = adjustcolor(yCols, alpha.f=0.6), 
#        xlab = TeX("$F^*$"), #'Xi',
#        ylab = TeX('$B^*/B_0$'), #'Zeta',
#	main = TeX("Bias in Estimated $B^*/B_0$"), #Relative Biomass", #Optimal Biomass",
#	ylim = c(zetaBot, zetaTop),
...
#quiver(
#        XStar[w,2][thin]*M, XStar[w,3][thin],
#        xBias[w][thin]*M, yBias[w][thin],
#        scale=0.025
#)
