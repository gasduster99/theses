	...
	#
		...
		if(!any(is.na(datGen$N)) & !any(datGen$N<0) & isGood(par, extra)){
			...
			optAns = fit$optimize(cpue,
                                ...
                                lower   = c(log(0.001), log(M), log(10^-40)), #log(1e-7)),
                                upper   = c(log(1), log(100), log(100)), #log(1e-2)),
                                gaBoost = list(run=100, parallel=FALSE, popSize=10^3),
                                persistFor = 1
                        )
			optAns = fit$optimize(cpue,
			        c('lsdo', 'lbeta'), #'lq'),
			        lower   = c(log(0.001), log(10^-40)), #log(1e-7)),
			...
			tryCatch({
				optAns = fit$optimize(cpue,
					c('lsdo', 'lalpha', 'lbeta'), #'lq'),
                                	lower   = c(log(0.001), log(M), log(10^-40)), #log(1e-7)),
                                	upper   = c(log(1), log(100), log(10^6)), #log(1e-2)),
					cov     = T
