...
FMsy = function(alpha, gamma, M){
	##
	#FUpper = P0 #alpha-M
	##root = uniroot.all(function(ff){  ((gamma-1)/alpha)^(gamma-1) - (M+ff)^(1/(gamma-1)) - (ff/(gamma-1))*(M+ff)^((2-gamma)/(gamma-1)) }, c(0, FUpper)) #1 - exp(log(ff)+log(alpha)+log(gamma) - (2*log(M+ff)+log(alpha/(M+ff)-1))) }, c(0, FUpper))
...
#alpha = 1 #3/5 #1
#gamma = 1.9   #1.9
beta  = P0 #getBeta(alpha, gamma, M, P0) #0.003
##alpha>M+ff
##FUpper = alpha-M
...
##
#if( !isGood(par, extra) ){
#	par = strongRoot(f, par, extra, howGood, lower=c(0, 1), upper=c(5, 5), monitor=T)
#}
##
#writeLines('\nRoots')
...
#
truth = prodModel$new(
	dNdt=dPdt, N0Funk=function(lalpha, lbeta, gamma, M){exp(lbeta)}, #PBar(0, alpha, beta, gamma, M)}, #model
        time=1:TT, catch=catch, M=M,			#constants
	lalpha=log(alpha), lbeta=log(P0), gamma=gamma,	#parameters
        lq=log(0.004), lsdo=log(0.01160256),		#nuisance parameters
...
curve(pellaSRR(x, exp(truth$lalpha), exp(truth$lbeta), truth$gamma), 0, P0*2)
abline(v=P0)
truth$plotMean()
points(dat)
...
#
fit = prodModel$new(
        dNdt=dPdt, N0Funk=function(lbeta){exp(lbeta)}, #PBar(0, exp(lalpha), exp(lbeta), gamma, M)}, #PBar(0, alpha, beta, gamma, M)}, #model
        time=1:TT, catch=catch, M=M,  			#constants
        lalpha=log(alpha), lbeta=log(P0), gamma=2,	#parameters
        lq=log(0.004), lsdo=log(0.01160256),			#nuisance parameters
        xi=xi, zeta=zeta				#other incidentals to carry along
)
#
optAns = fit$optimize(dat,
        c('lsdo', 'lalpha'), #lq is optimized via profile likelihood internally
        lower   = c(log(0.001), log(M)),
        upper   = c(log(1), log(100)),
        gaBoost = list(run=50, parallel=5, popSize=5*10^3),
	persistFor = 5
)
##
##optAns1 = fit$optimize(dat,
##        c('lbeta'), #lq is optimized via profile likelihood internally
##        lower   = c(log(10^3)),
##        upper   = c(log(10^4)),
...
##)
##
optAns2 = fit$optimize(dat,
        c('lsdo', 'lalpha', 'lbeta'), #lq is optimized via profile likelihood internally
        lower   = c(log(0.001), log(M), log(10^3)),
        upper   = c(log(1), log(100), log(10^4)),
        gaBoost = list(run=50, parallel=5, popSize=5*10^3),
	persistFor = 5
)
