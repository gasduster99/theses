...
D = D[!is.na(D$xiInv),]
##
png(sprintf('pellaDatP0%sMatern.png', P0))
...
#xi bias
png(sprintf("xBiasPellaP0%sMatern.png", P0))
#
...
#zeta bias
png(sprintf("yBiasPellaP0%sMatern.png", P0))
#
...
#euc bias
png(sprintf("directionalBiasPellaP0%sMatern.png", P0))
#
...
#
sqLoss = function(y, yStar, yVar, k){
        ...
        #
        out = sapply(k, function(kk){ sum(yVar + (kk/(kk+1))*(y-yStar)^2) })
        return(out)
...
yRes = y[take]
XRes = X[take,]
#axes = XRes[,2:3] #registerData(yRes, XRes, axes, Tg)
gpPredRes = gpPredict(XRes, XRes[,2:3], gpFit, asMat=F)
gpPredVar = diag(Tg)[take] #gpPredVar(XRes[,2:3], gpFit, asMat=F)
sq = sqLoss(yRes, gpPredRes, gpPredVar, 10^15)
##geoRes = as.geodata(cbind(XRes[,2], XRes[,3], yRes-gpPredRes))
png(sprintf('pellaResidualsP0%sMatern.png', P0))
##plot(geoRes, lowess=T)
plot(XRes[,2], XRes[,3], col=map2color(yRes-gpPredRes, hcl.colors(10, "Blue-Red 3", rev=F)), pch=19)
##points(D[,c("xiHat", "zetaHat")], col=map2color(D$minDist, hcl.colors(60, "Zissou 1", rev=T)))
dev.off()
#
##
#png(sprintf("pellaResP0%sMaternHist.png", P0))
#hist(yRes-gpPredRes)
#dev.off()
