...
dPdt = function(t, P, lalpha, lbeta, gamma, M, catch){
	...
	#linearly interpolate catches
        ...
        if(q==0){ C=Cl }
        #
	Fmsy = uniroot.all(function(ff){ 1 - exp(log(ff)+log(alpha)+log(gamma) - (2*log(M+ff)+log(alpha/(M+ff)-1))) }, c(0, exp(lalpha)-M))[1]
        if( is.na(Fmsy) ){ warning("Fmsy is NA") }
	#
...
b1 = cMax-slope*mid
b2 = 2*slope*mid + b1
FtFmsy = (slope*time+b1)*(time<=mid) + (-slope*time+b2)*(time>mid)
#
M = 0.2
#
P0 = 10000
...
#for(i in 1:length(zetaSims)){
	...
	#
		...
		par <- tryCatch({
			...
			par = strongRoot(f, par, extra, howGood, lower=c(0, 0), upper=c(5, 5), monitor=F, gaOnly=F, gaOptim=T)
		}, error=function(err){
			#
			print(err)
			#if that fails for some reason do gaOnly and no optimization
			par = strongRoot(f, par, extra, howGood, lower=c(0, 0), upper=c(5, 5), monitor=F, gaOnly=T, gaOptim=F)
		})
		...
		#
		writeLines(sprintf("PID: %s, Xi: %s, Zeta: %s -> (%f, %f, %f) -> (%f, %f) isGood: %d \n", Sys.getpid(), xiSims[j], zetaSims[i], alpha, beta, gamma, FStar/M, PStar/PZero, isGood(par, extra)))
		...
		datGen$printSelf()
		#print(datGen$N)
		#print(!any(is.na(datGen$N)) & !any(datGen$N<0))
		...
		if(!any(is.na(datGen$N)) & !any(datGen$N<0)){
			...
			optAns = fit$optimize(cpue,
                                ...
                                lower   = c(log(0.001), log(M), log(P0/10)), #log(10^-40)), #log(1e-7)),
                                upper   = c(log(1), log(100), log(1.5*P0)), #log(100)), #log(1e-2)),
                                gaBoost = list(run=100, parallel=FALSE, popSize=10^3),
                                persistFor = 1
                        )
			optAns = fit$optimize(cpue,
			        c('lsdo', 'lbeta'), #'lq'),
			        lower   = c(log(0.001), log(P0/10)), #log(10^-40)), #log(1e-7)),
			        upper   = c(log(1), log(1.5*P0)), #log(100)), #log(1e-2)),
			...
			tryCatch({
				optAns = fit$optimize(cpue,
					c('lsdo', 'lalpha', 'lbeta'), #'lq'),
                                	lower   = c(log(0.001), log(M), log(P0/10)), #log(10^-40)), #log(1e-7)),
                                	upper   = c(log(1), log(100), log(1.5*P0)), #log(10^6)), #log(1e-2)),
					cov     = T
