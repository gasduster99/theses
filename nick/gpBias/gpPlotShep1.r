...
getData = function(dir, xiSims, zetaSims){
	...
	for(i in 1:length(zetaSims)){
			...
			if( file.exists(fileDat) & file.exists(fileFit) ){
				...
				md = optimize(myDist, c(0, dat$xi), xi=dat$xi, zeta=dat$zeta)$objective				
...
xiTop = 3.5
#
zetaSims = seq(0, 0.65, 0.05) #seq(0, 0.65, 0.05) #seq(0.1, 0.8, 0.01) #seq(0.1, 0.8, 0.05) 	#rev(seq(0.1, 0.80, 0.01)) #
xiSims =  seq(0.5, 2.5, 0.5) #seq(0.5, 3.5, 0.5) #seq(0.5, 3.5, 0.05) #seq(0.5, 3.5, 0.25)		#seq(0.5, 3.5, 0.05)       #
...
##add to tame hot spot
#dat = readRDS('modsShepFineQFixBFix/datGen_xi3.5_zeta0.2.rda')
#fit = readRDS('modsShepFineQFixBFix/fit_xi3.5_zeta0.2.rda')
##the inversion actually found
#FInv = FMsy(dat$alpha, 1, M)
#xiInv = FInv/M
#zetaInv = PBar(FInv, dat$alpha, dat$beta, dat$gamma, M)/PBar(0, dat$alpha, dat$beta, dat$gamma, M)
...
##NOTE: replace with a propper observation uncertainty
#if(length(fit$rsCov)==0){ v=0 }else{ v=getlFV(fit)$lFV }
#Dall = rbind(Dall, c(dat$xi, dat$zeta, xiInv, zetaInv, xiBH, zetaBH, md, log(Fs), v))

#
D = Dall
cut = 10
D = D[D$xiBH<cut & D$xiInv<4 & D$zetaInv<0.7,]
...
#
#xAug = 2.75
xAug = seq(0.75, 4, 0.3) #seq(0.75, 4, length.out=10) #
#xAug = c(xAug, seq(7/8, 3, xiRes)) #seq(7/8, 4.5, xiRes)
aug = cbind(rep(1, length(xAug)), xAug, 1/(xAug+2)) #rep(0.5, length(xAug)))
#
X = rbind(X, aug)
y = c(y, log(xAug*M))
...
gpFit = gpMAP(par, hessian=F, psiSample=F, lower=c(0.1, rep(eps(), 2))) #rep(eps(), 3)) #
print(gpFit)
#print(gpFit)

#prediction
zetaStar = seq(min(D$zetaInv), max(D$zetaInv), 0.005) 	#seq(0.15, 0.35, 0.001)  #rev(seq(0.1, 0.80, 0.01)) #
xiStar   = seq(min(D$xiInv), max(D$xiInv), 0.01)  	#seq(1, 3.5, 0.005)
...
curve(1/(x+2), from=0, to=4, lwd=3, add=T) 
w = (XStar[,2]>xiBot & XStar[,2]<xiTop & XStar[,3]>zetaBot & XStar[,3]<zetaTop) 
thin = c(T,rep(F, 130)) #rep(F,60))
quiver(
...
#take = X[,3]!=0.5
take = rep(F, length(y))
take[1:(length(take)-length(xAug))] = T
yRes = y[take]
XRes = X[take,]
gpPredRes = gpPredict(XRes, XRes[,2:3], gpFit, asMat=F)
png(sprintf('shepResidualsP0%s1.png', P0))
