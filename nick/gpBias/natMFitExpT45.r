...
binTrk = 3 #ceiling(abs(log10(minDiff)))

#
#SIM
#

##./modsSchnuteExpT45N150Wide/datGen_xi1.94_zeta0.363.rda
xiBase = 1.94
zetaBase = 0.363

#
baseFitName = sprintf('./modsSchnuteExpT45N150Wide/fit_xi%s_zeta%s.rda', round(xiBase, binTrk), round(zetaBase, binTrk))
#
...
if( F ){
		...
		datGen = prodModel$new(
                ...
                datName = sprintf('%s/datGen_xi%s_zeta%s_M%s.rda', place, round(xiBase, binTrk), round(zetaBase, binTrk), m)
                datGen$save( datName )
        }
}else{
...
#for(i in 1:length(datFiles)){
	...
	fit = prodModel$new(
	        dNdt=dPdt, N0Funk=function(lalpha, lbeta, gamma, M){PBar(exp(lalpha), exp(lbeta), gamma, 0, M)}, #model
	        time=1:TT, catch=FtFmsy, M=datGen$M,				     #BH	#constants
	        alpha=datGen$alpha, beta=getBeta(datGen$alpha, -1, datGen$M, P0), gamma=-1, 	#parameters
		lalpha=datGen$lalpha, lbeta=log(getBeta(datGen$alpha, -1, datGen$M, P0)), 	#reparameterize
	        lq=log(0.00049), lsdo=log(0.01160256), 		#nuisance parameters
	...
	optAns = fit$optimize(cpue,
	        c('lsdo', 'lalpha'), 
	        lower   = c(log(0.001), log(datGen$M)), 
	...
	optAns = fit$optimize(cpue,
	        c('lsdo', 'lalpha', 'lbeta'), 
	        lower   = c(log(0.001), log(datGen$M), -10), 	
