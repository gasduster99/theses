...
logML = function(x, fb){
	#
	l1=x['l1']; l2=x['l2']; s2=x['s2']; g=x['g']
	#     from like             from beta integral
	out = -0.5*logDet(fb$V) + -0.5*logDet(t(fb$LIX)%*%fb$LIX) + -0.5*fb$S2 + 
		s2Prior(s2) + l1Prior(l1) + l2Prior(l2) + gPrior(g)#+ thPrior(th) + nuPrior(nu)
	#
	return(out)
}

#
funnyBusiness = function(l1, l2, s2, g){	
	#
	SIG = s2*powExpCor(dMat1, l1)*powExpCor(dMat2, l2) + g*Tg	
	#
	L  = t(chol(SIG))
...
#
ga=2; gb=1
gPrior = function(g, log=T){
	#
        out = dinvgamma(g, ga, gb, log=T)
        if( !log ){
                out = dinvgamma(g, ga, gb, log=F)
        }
...
#
registerData = function(y, X, axes, Tg, dMat1=dist(axes[,1]), dMat2=dist(axes[,2]), l1a=.GlobalEnv$l1a, l1b=.GlobalEnv$l1b, l2a=.GlobalEnv$l2a, l2b=.GlobalEnv$l2b, s2a=.GlobalEnv$s2a, s2b=.GlobalEnv$s2b, ga=.GlobalEnv$s2a, gb=.GlobalEnv$gb){ #nua=.GlobalEnv$nua, nub=.GlobalEnv$nub, tha=.GlobalEnv$tha, thb=.GlobalEnv$thb
	...
	.GlobalEnv$ga=ga;   .GlobalEnv$gb=gb 
}
...
gpMAP = function( par, lower=rep(eps(), 3), upper=c(Inf, Inf, Inf), hessian=F, psiSample=F ){	
	...
	realPar['g'] = log(realPar['g'])
	#
	opt = optim(realPar,
        	function(lp){
			p = c(exp(lp['l1']), exp(lp['l2']), exp(lp['s2']), exp(lp['g']))
			fb = funnyBusiness(p['l1'], p['l2'], p['s2'], p['g']) 
			return(-logML(p, fb))
        	},
	...
	outPar['g']  = exp(outPar['g'])
	#
	fb = funnyBusiness(outPar['l1'], outPar['l2'], outPar['s2'], outPar['g'])
...
#
gpPredictVar = function(XStar, axeStar, gpFit, asMat=T){
	#
        XStar = as.matrix(XStar)
        SIGStar = powExpCor(rectDist(as.matrix(axeStar[,1]), as.matrix(axes[,1])), gpFit$psi['l1'])
        SIGStar = SIGStar * powExpCor(rectDist(as.matrix(axeStar[,2]), as.matrix(axes[,2])), gpFit$psi['l2'])
	#
	SIGSStar = powExpCor(rectDist(as.matrix(axeStar[,1]), as.matrix(axeStar[,1])), gpFit$psi['l1'])
        SIGSStar = SIGSStar * powExpCor(rectDist(as.matrix(axeStar[,2]), as.matrix(axeStar[,2])), gpFit$psi['l2'])
	#
	SIG = powExpCor(dMat1, gpFit$psi['l1'])*powExpCor(dMat2, gpFit$psi['l2'])
	#KPP-KPX%*%KInv%*%KXP
	#self$S2(XX, XX) - self$S2(self$X, XX)%*%self$KInv%*%self$S2(XX, self$X)
	out = (SIGSStar - SIGStar%*%strongInv(SIG)%*%t(SIGStar))*gpFit$psi['s2'] + gpFit$psi['g']
	#
        if( asMat ){
                out = matrix(out, nrow=length(unique(axeStar[,1])), ncol=length(unique(axeStar[,2])))
        }
...
#
##
#par=c(l1=0.5, l2=0.5, s2=0.28, g=1)
