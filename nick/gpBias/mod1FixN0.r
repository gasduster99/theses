...
mod = prodModel$new(dNdt=dNdt, time=1:TT, N0Funk=N0Funk, delta=1-exp(-M), catch=catch, 
	Cs    = 290.4111,	#mean(catch), 		# 263.9692
	cllhs = -0.3287746,	#cloglog(0.45),
	gamma = -1.17, #4671,	#-1,
	lq    = -7.622283,	#log(0.0004271659), 	#q = 0.00047, 
	lsdo  = -2.155007	#log(0.146974)
)
...
optAns = mod$optimize(cpue,
        c('lsdo', 'Cs', 'cllhs', 'lq', 'gamma'),
        lower   = c(log(0.001), min(catch)*0.5, cloglog(0.001), log(1e-7), -1.1719768014),
        upper   = c(log(1), max(catch), cloglog(0.99), log(1e-2), 1),
