...
%thr = kr*(k-kr)/((z+kr)^2)
%
\newcommand{\kr}{ \frac{\kappa w_\infty}{w(a_0)} }
\newcommand{\one}{
	\left(\frac{Z(Z+\kappa)}{\alpha w(a_0)(Z+\kr)}\right)^\gamma
}
\newcommand{\two}{
	\left(\frac{\gamma F}{\alpha w(a_0)}\right) \left(\frac{Z(Z+\kappa)}{\alpha w(a_0)(Z+\kr)}\right)^{\gamma-1}
}
\newcommand{\thr}{
...
%
\newcommand{\oneA}{
	\left(\frac{Z^*(Z^*+\kappa)}{w(a_0)(Z^*+\kr)}\right)^\gamma
}
\newcommand{\twoA}{
	\left(\frac{\gamma F^*}{w(a_0)}\right) \left(\frac{Z^*(Z^*+\kappa)}{w(a_0)(Z^*+\kr)}\right)^{\gamma-1}
}
\newcommand{\thrA}{
	\frac{\left(\kr\right)\left(\kappa-\kr\right)}{(Z^*+\kr)^2}
}
...
\begin{itemize}
\item knife-edge recruitment at age $a_0$.
\item All fish of at least $a_0$ years old are equally vulnerable to fishing and natural mortality with constant $M$.
\item Von Bertalanffy individual growth \shortciteA{von_bertalanffy_quantitative_1938}
\end{itemize}

%
The foundation of the delay model is the assumption of linear individual growth 
rate with age. % in individual growth.
...
growth (in weight) with age. $w_\infty$ is the maximum weight of an individual fish in 
the population, and $w(a)$ is the average weight of an individual at age $a$.
Solving the above ODE for w(a), leads to an asymptopting exponential growth curve in weight. 
...
&\frac{dB}{dt} = w(a_0)R(B;\theta) + \kappa \left[w_\infty N-B\right] - (M+F)B \label{bEq}\\
...
%
\begin{align}
\bar{B}(F) = \frac{1}{\beta\gamma} \left( 1 - \Big(\frac{(F + M) (F + M + \kappa)}{\alpha w(a_0)(F + M + \kr)}\Big)^\gamma\right)
\end{align}
...
%&= (1-\left(\frac{(F+M)*(F+M+\kappa)}{\alpha*(F*w(a_0)+M*w(a_0)+\kappa*w_infty)}\right)^\gamma-(F*(((F+M)*(F+M+\kappa))/(\alpha*(F*w(a_0)+M*w(a_0)+\kappa*w_infty)))^(\gamma-1)*\gamma*(\alpha*(F*w(a_0)+M*w(a_0)+\kappa*w_infty)*(2(F+M)+\kappa)-(F+M)*(F+M+\kappa)*\alpha*w(a_0)))/(\alpha*(F*w(a_0)+M*w(a_0)+k*w_infty))^2)/(\beta\gamma) \label{dBdFS}.
%&= ( 1-\Big(\frac{(F+M)(F+M+\kappa)}{\alpha w(a_0)(F+M+\kappa w_\infty/w(a_0))}\Big)^\gamma - ( F*( ((F+M)*(F+M+\kappa))/(\alpha*(F*w(a_0)+M*w(a_0)+\kappa*w_infty)) )^(\gamma-1)*\gamma*(\alpha*(F*w(a_0)+M*w(a_0)+\kappa*w_infty)*(2(F+M)+\kappa)-(F+M)*(F+M+\kappa)*\alpha*w(a_0)) )/( \alpha*(F*w(a_0)+M*w(a_0)+k*w_infty))^2)/(\beta\gamma) \\
\end{align}
\endgroup
...
\beta &= \frac{1}{\gamma B_0} \left( 1 - \Big(\frac{M (M + \kappa)}{\alpha w(a_0)(M + \kr)}\Big)^\gamma\right)\\
\frac{B^*}{B_0} &= \frac{ 1 - \Big(\frac{(F^* + M) (F^* + M + \kappa)}{\alpha w(a_0)(F^* + M + \kr)}\Big)^\gamma }{ 1 - \Big(\frac{M (M + \kappa)}{\alpha w(a_0)(M + \kr)}\Big)^\gamma }
\end{align}
\endgroup
