...
\item Methods
	\begin{itemize}
	{\color{gray}\item State the Bio Model (Pella v. Schaffer)}
...
\item Proposal (<6 pages)
	\begin{itemize}
	\item challanges of filling out response space with three parameter SRR (numerical methods)
...
%NOTE: we can nnot observe all fish in the sea, and this rely on an index of abundance 
%which typically comes from experimental fishing proceedure(for example ccfrp).
The data for a typical surplus-production model comes in the form of a time 
series of observations of an index of abundance for some population of interest. 
The index is often observed alongside a varienty of other known quantities, 
...
%
The observed indicies are assumed to have multiplicative log-normal errors, and thus
...
\end{equation}
%{\color{red}$C=FB$ to set up yeild calculations later
...
other naturally occuring sources of mortality other than the recorded fishing 
...
perspective a major goal of the model is to accuratly infer a quantity known as 
\emph{maximum sustainable yeild} (MSE). One could maximize simple yeild at a 
...
accomplished by maximimzing the equilibrium level of catch (or yeild) over 
time. Equilibrium yeild is considered by replacing the steady state biomass 
...
(\ref{ptRef}). MSY is found by optimizaing $\bar C(F)$ with respect to $F$, 
and $F^*$ is the fishing rate at MSY. Going forward let $^*$ decorate any value 
derived under the condition of MSY. 

% to continue the species and produce  not only are there  however this has the {\color{red}a description of maximum sustainable yeild}

%
The canonical production model in fisheries is the Schaeffer model. The 
Schaeffer model is formed by choosing $R$ to be logistic growth parameterized 
so that $\bm{\theta} = [r, K]$ and the family takes the following form,  
%
\begin{align}
R(B; [r, K]) = r B \left(1-\frac{B}{K}\right). \label{logistic}
\end{align}
$r$ is parameter controlling the average reproducive rate of the population. 
...
%{
%\color{red} 
%Concept of Maximum sustainable yeild (MSY)
%writtting sustainable yeild $\bar Y(F) = F\bar B(F)$.
%}

%
Logistics SRR produces idealized parabolic recruitment with equilibrium 
...
maximized by fishing at the peak productivity of the stock. {\color{orange}By symmetry 
it is clear that this peak occurs at $B^*=\frac{K}{2}$. The fishing rate required 
to position the stock at MSY is $F^*=\frac{r}{2}$, which is half of the stock's average 
reproductive rate at the origin. In the absence of fishing $\frac{dB}{dt}$ woudl be driven entirly by $R$We can also notice that at MSY the $\frac{dB}{dt}$}  

%
While this idealized form is instructive, and convienient, these simplistic 
dynamics are also potentially problematic. The symmetry of the logistic 
functional form is very rigid in that it assumes dynamics in the lower (growth 
...
{\color{red}Furthermore, and more practically, the fishery is managed based on
...
$(\xi, \zeta)\in \left(\mathbb{R}^+, \frac{1}{2}\right)$
}

{\color{red}Beverton Holt and Ricker Introduction
In practice, at this time, the SRR is typically chosen to depend only on 
two parameters. Which struggle similarly to model the full space of referece points.
}

Below I present the three parameter Pella-Tomlinson (PT) family, which has a 
convinient form that includes the logistic SRR as a special case to form the 
Schaeffer model. The Pella-Tomlinson SRR is parameterized so that 
$\bm{\theta} = [r, K, \gamma]$ and the family takes the following form, 
%Pella Tomlinson
\begin{align}
...
%{\color{red} And $\gamma$ is a parmeter which adds 
%flexibility to the fiamily by allowing for compensitory or non-compensitory 
%growth. (break symetriy... get better description) }
...
parameter allows enough flexibility fully describe the space of reference points 
used in management $(\xi, \zeta)\in \mathbb{R}^+_2$. To see this, the reference 
points are analytically derived for the PT model in the following section.
...
%
Under PT recuitment the process model is defined by the following ODE,
...
convienient to notice that $\bar B(0)=K$ to arrive at the same result. The 
expression for $B^*$ is given by evaluating Eq(\ref{Beq}) at $F^*$.

%
To get an expression for $F^*$, the equilibrium yeild is maximized with respect to $F$,
\begin{equation}
F^* = \argmax_F F\bar B(F).
\end{equation}
%
For PT maximization can be done analytically, however many three parameter SRR's 
do not result in tractable analytical solutions. For PT proceeds by differentiating 
the equilibrium yeild with respect to $F$ as follows,
%{
%\color{red} 
%Concept of Maximum sustainable yeild (MSY)
%writtting sustainable yeild $\bar Y(F) = F\bar B(F)$.
...
expression for the fishing rate required to produce MSY, {\color{red}Below any quantity 
evaluated at MSY shall be decorated with $~^*$.}
%
\begin{align}
...
following expression for biomass at maximum sustainable yeild, 
\begin{align}
...
%Eq(\ref{xizetaSimple}) can then take a specific analytical form in terms of the 
  %an expression for $\xi$ and $\zeta$ can be found in terms of model paramters.  
...
%parameter SRR.\\ Collapse to the case of Schaffer. Similarly to Eq(\ref{
%logisticSRR}) setting $\gamma=2$ results in the equilibrium equaitons for the 
%restricted schaffere model.  
%} 

%speaking it is not possible to analytically invert this  
Generating simulated indicies of abundnance from the PT model requires 
...
proceedure is often extremely unstable. That said, for the case of PT this 
...
%
Indicies are generated under the following condistions. A regular grid of 
biologically important values for $\xi$ and $\zeta$ are considred. For each 
...
typically small value of 0.0005. $\sigma$ is fixed at the relatively samll 
value of 0.01 to focus specifically on the behaviour of population parameters.
...
produces the Schaffer model {\color{red}(cite)}. 
...
{\color{red} Plot a Representative PT SRR, next to a Schaffer (logistic) SRR}
