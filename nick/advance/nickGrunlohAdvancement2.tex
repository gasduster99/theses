...
%
\begin{equation}
\frac{dB}{dt} = P(B; \bm{\theta}) - C. \label{ode}
\end{equation}
...
Schaeffer model is formed by choosing $P$ to be logistic growth ({\color{red} cite}) parameterized 
...
$\frac{dB}{dt}$ would be driven entirely by $P$, but at MSY $\frac{dB}{dt}=0$ 
...
\begin{itemize}
\item introduce fisheries type data
\item ODE modeling formulation
\item Schaeffer Model (Logistic production function)
\item Pella-Tomlinson production function
\item Reference Points
\item general simulation idea
\end{itemize}
...
%
\begin{figure}[h!]
\includegraphics[height=0.5\textheight]{../present/shaeferDat.png}
\caption{
...
%
In Appendix (\ref{appB}) a profile likelihood method for estimating all of the 
...
%
\subsection{integrating ODEs, Stiffness, and Interpolation.}
...
\clearpage
\section*{Appendix B: Profile Likelihood MLE \label{appB}}
