...
\title{Meta-Modeling for Bias Estimation of Biological Reference Points Under the Schaefer Model}
...
the functional form, rather than a biological characteristic of the stock. Mangel et al. 
note that use of a three-parameter stock-recruitment relationship allows for 
independent estimation of these reference points. This research seeks to 
understand the nature of biases in reference points resulting from fitting a 
two-parameter logistic functional form when the true relationship follows a 
three-parameter stock-recruitment relationship. This study's findings suggest 
the useful limits of the misspecified Schaeffer model, and mechanisms of model 
failure which arise from mapping a three-dimensional parameter space into two 
dimensions.
%ecification of this type 
...
The most fundamental model in modern fisheries models is the surplus-production
model. Data for a typical surplus-production model comes in the form of an index 
...
in this setting is defined as the net biomass increase due to all birth, 
maturation, and migration processes accounting for all naturally occurring 
...
While catches (aka yields) are observable quantities \shortcite{sen_sampling_1984, pearson_documentation_1997}, 
...
Section (\ref{ptRef}). MSY is found by optimizing $\bar C(F)$ with respect to 
...
parameterized so that $\bm{\theta} = [r, K]$ so that the family of production 
...
in the absence of competition for resources (i.e. the slope of SRR at the 
origin). $K$ is the so called "carrying capacity" of the population. In this 
context the carrying capacity can be formally stated as steady state biomass 
in the absence of fishing (i.e. $\bar B(0)=K$).  
...
stock's maximum reproductive rate at the origin. 
...
Fisheries are very often managed based upon reference points (RP) which serve as 
simplified heuristic measures of population behavior. The mathematical form of 
RPs depends upon the model assumptions primarily through the production function. %({\color{red} cite}). 
...
at MSY. In general $F^*\in\mathbb{R}^+$ and $\frac{B^*}{\bar B(0)}\in\left(0, 1\right)$, 
...
The basis of the Schaeffer model is ripe with debate \shortcite{kingsland_refractory_1982},
...
%
The bias-variance tradeoff \shortcite{ramasubramanian_machine_2017} makes it 
clear that the addition of a third parameter in the production function will 
neccessarily reduce estimation bias. However the utility of this bias reduction 
is still under debate because the particular mechanisms and behavior (direction and magnitude) %and mechanisms %behavior 
of these biases for key managment quantities %(and the mechanisms of inducing these biases) 
are not fully understood or described. \shortciteA{lee_can_2012} 
provides some evidence that steepness estimation, and thus RPs via \shortcite{mangel_perspective_2013}, 
is dependent on model misspecification as well as data lacking contrast in 
biomass. However the particular behaviors of these biases are not fully described.

%Add GOAL of understanding directionality and mechanism of bias.
%Add claim of novilty by reconizing that bias is a function of RP, a flexible method of propogating estimation uncertainty and accounting for nonlinear bias patterns.
Together the general behavior of the PT model and the pragmatic understandable 
simplicty of the Schaefer model make the PT/Schaefer pair an ideal setting for 
...
data are simulated from the three parameter PT production model but fit with the  
the two parameter Schaefer model. 

%
A key insight of this work is that bias is considered broadly across RP-space to 
uncover patterns of bias magnitude and direction. A flexible Gaussian Process (GP) 
meta-modeling approach is used to model bias patterns across RP-space. This 
technique not only informs the utility of reducing bias, but also suggest mechanisms 
for understanding what causes bias. Further, the effect of contrast on estimation 
is considered alongside the effect of model misspecification.  

%
{\color{gray}
\begin{itemize}
\item[\checkmark] Lead into this reaserch with other studies (Lit review)
\item Enphasize a clear statement of the study goal
\item a statement of novilty
	\begin{itemize}
	\item[\checkmark] model misspecification leads to bias
	\item[Sorta] nonconstant bias of RP-space (a wholistic summary of model performance.)
...
%{\color{gray}
%%
%Scaeffer model clearly insufficient but can be made to estimate some parameters correctly Prager 2002, Prager 2003.
...
%
%%
%PT generalizes schaeffer Maunder 2003
%Three parameter PT should supplant Schaefer
%
%%
%Other functions recommended in practice BH Punt 2005 or Ricker. 
%Although statistcally differenctiating the models is difficult Dorn 2002, Brodziak 2002 
%BH and Ricker suffer from the similare model deficiencies Mangle 2013 as the Scaeffer does for the PT relating the reduced decrees of freedom in the parameter space.
%
%%
...
%Maunder 2003
%	PT generalizes and should supplant Schaeffer
%	"shape" parameter B*/B0
%	Bias vairance trade off between PT3 Vup/Bdown and Logistic Vdown/Bup
%Francis 2012/Lee 2011
%	assessment model correctly specified
%Lee 2011
%	contrast estimate steepness
%Field 2009 Bacaccio BH v. Stanly 2009 Ricker 
%zeta=0.39		  zeta=0.5
%
%%
%\begin{itemize}
...
%SRR model-misspecification with special interest on RF management quantities.  
...
SRR as a special case to form the Schaefer model. The Pella-Tomlinson SRR is 
...
logistic curve. The parameters $r$ and $K$ maintain the same interpretation as 
...
derived for the PT model below. % in the following section.
...
numerical methods to invert; more over the numerical inversion procedure is 
often extremely unstable. That said, for the case of PT this relationship is 
...
include the majority of most commonly assessed fish species {\color{red}cite}. 
...
the associated pair $(r, \gamma)$ are computed from Eq (\ref{rg}). Since $K$ 
...
The red X's indicated 4 simulation locations where the Schaefer model is a particularly 
misspecified.
}
\end{minipage}
\end{figure}
...
parameters \shortcite{hilborn_quantitative_1992} {\color{red} cite walters}). In particular it is 
understood that catch data must induce "contrast" in {\color{orange} index data
} so as to well inform $r$. It is not well understood how contrast may factor 
into biases induced by model misspecification. To investigate the role that 
...
$\frac{F(t)}{F^*}$, catch can then be written as $C(t)=F^*\left(\frac{F(t)}{F^*}\right)B(t)$.  
...
$B^*$. For constant $\frac{F(t)}{F^*}$ the Schaefer model has an analytical 
solution, via integrating factors, so that $B(t)$ comes to equilibrium 
exponentially. The relative fishing rate is defined on $[0, \infty)$; when 
...
via the Livermore Solver (lsode), and catch is linearly interpolated between sampled 
epochs. More detail about issues of continuity and ODE integration are described in 
Appendix (\ref{appODE}).
...
Thus, let $I_t$ be an observation of PT index data at time $t$. The observation 
...
effect of rescaling biomass which can often function similarly the role of the 
...
to yield fast and reliable inference \shortcite{deyoreo_integrating_nodate}. %({\color{red} Cite Maria DeYorio pdf}).
...
Figure(\ref{rpGrid}), a Gaussian Process (GP) model is used as a flexible, 
stochastic interpolator over RP space. As previously established, in 
Section (\ref{modelFit}), the biological parameters of interest are the Schaefer model's 
$log(r)$ and $log(K)$ parameters. Since the estimates of these parameters are 
random variables, with variances given by the inverse of the observed fisher 
information, interpolation of MLEs requires paying additional attention to 
propagating estimates of uncertainty into the metamodel. 

%{\color{red} Motivate gaussianity?}
...
generated across the broader space of RPs are mapped onto the limited space of 
...
%
\clearpage
\subsection{More Typical Catch Histories}
...
fishing rate is a somewhat oversimplified setting. It is far more 
common to observe fishing increase rapidly as technology and fishing techniques 
improved around the turn of the century, followed by a rapid drop in fishing 
as management proceedures are applied ({\color{red}cite}). Figure(\ref{expCatchT45}) 
demonstrates a more realistic, while still simplified, fishing behavior that 
exposes the population to a variety of fishing rates, which induce contrast in 
the generated indicies and allow the fitting model to observe the population 
decrease in size and subsequently rebuild. This represents a "high contrast" 
setting that is known to better inform growth rate parameters, such as $r$.
 
%\clearpage
\begin{figure}[h!]
\vspace{-1cm}
\begin{minipage}[h!]{0.49\textwidth}
\includegraphics[width=.85\textwidth]{../../../theses/nick/gpBias/bMSYRelBiasPellaExpT45.png}
\end{minipage}
\begin{minipage}[h!]{0.49\textwidth}
\includegraphics[width=.85\textwidth]{../../../theses/nick/gpBias/fMSYRelBiasPellaExpT45.png}
\end{minipage}
\caption{ \label{expT45BF} 
...
under the high contrast setting. On the one hand, notice the relative lack of 
bias in $F^*$ over a large swath of RPs far from the Schaefer line. On the 
other hand, notice that bias in $B^*$ increases here relative to the low contrast 
setting. The pattern of bias in $B^*$ %over the space of misspecified RPs in $B^*$ 
maintians a similar pattern, and overall scale, as the low contrast setting 
...
%
If the data is augmented so that the fishing rate is held at $F^*$ for an 
additional 15 time epocs, so that slower growing stocks may observe more data 
near $B^*$, Figure(\ref{expT60BmFm}) shows the updated bias surfaces. The scale
of bias in $B^*$ is reduced, but the scope and general pattern of bias remains 
similar. While the bias behavior of $B^*$ estimates are somewhat diminished, 
...
%
%\begin{figure}[h!]
\includegraphics[width=.85\textwidth]{../../../theses/nick/gpBias/bMSYRelBiasPellaExpT60.png}
...
%%
%\begin{figure}[h!]
\includegraphics[width=.85\textwidth]{../../../theses/nick/gpBias/fMSYRelBiasPellaExpT60.png}
%\caption{ \label{expCatchT45Bmsy}
%}
\end{minipage}
\caption{\label{expT60BmFm}
