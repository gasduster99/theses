...
model. Data for a typical surplus-production model comes in the form of a time 
...
the "biological parameters" entering the model thru a process model on biomass.
...
maturation, and migration processes after accounting for all other naturally 
occurring sources of mortality other than the recorded fishing from humans. 
The production function is assumed to be parametric function that relates the 
...
($C(t)$). While catches are observable quantities \shortcite{sen_sampling_1984, pearson_documentation_1997}, 
...
or yield) over time. Equilibrium yield is considered by replacing the steady 
state biomass ($\bar B$) in the assumed form for catch, so that $\bar C = F\bar B(F)$, 
...
Section (\ref{ptRef}). MSY is found by optimizing $\bar C(F)$ with respect to $F$, 
...
Schaeffer model is formed by choosing $P$ to be logistic growth \shortcite{mangel_theoretical_2006} %({\color{red} cite}) 
...
\end{align}
$r$ is parameter controlling the maximum reproductive rate of the population 
in the absence of competition for resources (i.e. the slope of SRR at the 
origin).  $K$ is the so called "carrying capacity" of the population. The 
carrying capacity can be formally stated as steady state biomass in the 
absence of fishing (i.e. $\bar B(0)=K$).  
...
\caption{
	\\The logistic production function in black plotted next to depictions of the key biological parmaeters and reference points.
	The slope at the origin (and thus $r$) is shown in blue, catch at MSY in red, biomass at MSY in green, and $K$ in purple at the right x-intercept.	
...
recruitment is observed when $B\in(0, K)$. Due to the parabolic shape of the 
logistic production function it is straightforward to see that yeild is maximized 
...
While this idealized form of the logisitc production function is instructive, and 
...
%
$F^*$ is the afore mentioned fishing rate at MSY. $\frac{B^*}{\bar B(0)}$ is the 
depletion of the stock at MSY. That is to say $\frac{B^*}{\bar B(0)}$ describes 
the fraction of the population to fish (or rather not to fish) to result in MSY.  
...
In general $F^*\in\mathbb{R}^+$ and $\frac{B^*}{\bar B(0)}\in\left(0, 1\right)$, 
...
only on two parameters. The Schaeffer model as presented depends only on 
...
%
In this study I consider the behavior of inference when using the Schaeffer model 
to fit data that are generated from only a slightly more complex production 
function which can full accomodate RP space. Namely data are simulated from the %can full accomidate the full space of RPs. I consider data simulated from the 
three parameter Pella-Tomlinson (PT) production model. Since the Schaeffer model 
is more restrictive than the PT model, estimates of RPs arrived at from the Schaeffer 
model must neccessarily be biased realative to the true PT RPs. Given the ubiquity 
of two parameter productions functions in management it is of paramount 
imporantance to understand the biasing lens through which we've come to understand 
our fisheries.

%and subsequently fit these 
%data using the two parameter Schaeffer model to observe the consequences of 
...
%\item[\checkmark] Schaeffer Model (Logistic production function)
...
relationship for many three parameter production functions{(\color{red}cite Derizo paper)}. 
...
include the majority of most commonly assesed fish species {\color{red}cite}. 
The red X's in Figure (\ref{rpGrid}) show four 4 simulation locations where the 
Schaeffer model is misspecified to a large degree and will be considered in more 
detail in {\color{red}Section()}. For each $\left(F^*, \frac{B^*}{\bar B(0)}\right)$, 
...
The horizontal line shows the constrained space of RPs for the Schaeffer model.
The red X's indicated 4 simulation locations where the Schaeffer model is a particularly 
misspecified.
}
\end{minipage}
\label{rpGrid}
...
parameters ({\color{red} cite hilborn, cite walters}). In particular it is 
...
To control $F(t)$ with respect to $F^*$, $C(t)$ is specified by a defining the 
quantity $\frac{F(t)}{F^*}$ as the realative fishing rate. $B(t)$ is defined 
by the solution of the ODE, and $F^*$ is defined by the biological parameters 
of the model, see Eq({\color{red}PT, Schaeffer}).  Thus by defining 
...
$B^*$. For constant $\frac{F(t)}{F^*}$ the Schaeffer model has an anylitical 
solution, via integrating factors, so that $B(t)$ comes to equilibrium 
exponenitally. The relative fishing rate is defined on $[0, \infty)$; when 
...
as would be done in practice however, the simulation can encounter a variaty of issues 
working with the naively discretized ODE. As a result the ODE is integrated implicatly 
via the Livermore Solver (lsode), and catch is linearly interpolated between sampled 
epocs. More detail about issues of continuity and ODE integration are described in 
Appendix (\ref{appODE}).
...
two parameter Schaeffer model behave under MLE inference when fit to PT data. 
...
For the Schaeffer model $\bm{\theta}=[r, K]$, and $B_t(\bm{\theta})$ is defined 
...
of the parameters into the log scale improves the relability of optimization
...
is found. The R package GA ({\color{red} cite GA}) is used to run a genetic 
algorithm to explore parameter space globally. Optimization occasional jumps into 
the L-BFGS-B local optimizater to refine optima within a local mode. The scheme 
functions by searching globally to iteratively improving hot starts for the local 
optimizer. 
...
identifiable. Posing the model in a Bayesian context provides a convienient 
...
to yeild fast and reliable inference ({\color{red} Cite Maria DeYorio pdf}).
...
stochastic interpolator over RP space. As previously estabilised, in 
Section (\ref{}), the biological parameters of interest are the Shaeffer model's 
...
propogating estimates of uncertainty into the metamodel. 
...
used to extend analysis of all major biological RP over the simluated grid. 
...
\end{align}
%
The GP residual variation provides an ideal mechanism for propogating uncertainty 
...
sampling distribuition uncertainty. This has the effect of smoothing the GP 
model in a way similar to the knugget effect. ({\color{red} cite knugget paper})
% from inference in thesimulation step. 
...
%The GP residual 
%variation provides an ideal mechanism for propogating this uncertainty while 
...
\begin{align}
        ...
        \check{MSY} = \frac{\check r \check K}{4} %\check r \check B^* \left(1-\frac{\check B^*}{\check K}\right)
\end{align}
