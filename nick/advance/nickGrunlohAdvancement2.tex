...
\shortcite{mangel_perspective_2013}. The limited RP-space of two parameter models
...
%The ODE equilibrium equations Reference points (RP) are key mangeare derived quantities The equilibrium equation the ODE   
%
%%
...
%
%%and conversly a two parameter production function will have estimation errrosd. 
...
%dominated by bias.
%
%as to overly limit estimate varianceso that estimation error is unduly dominated by bias.
%
%estimation errors of the two parameter Schaeffer model are unduly dominated by bias
%
% sincethe limited parameter 
...
evidence that steepness estimation, and thus RPs via \shortcite{mangel_perspective_2013}, 
is dependent on biomass contrast as well as model specification. % as well as contrast in data %lacking contrast in biomass. 
\shortciteA{conn_when_2010} comes to similar conclusions about steepness estimation
via calibration modeling techniques. Despite this understanding of steepness estimation
the implications have not been extended to a joint description of biases on the scale of 
management RPs. % biases jointly.

%The particular behaviors of thesie RP biases, 
...
GP meta-model is explicate about trade-offs between RPs %to the surface bias tradeoffs to the surface to 
...
%{\color{gray}
%%
%Schaeffer model clearly insufficient but can be made to estimate some parameters correctly Prager 2002, Prager 2003.
...
%BH and Ricker suffer from the similar model deficiencies Mangle 2013 as the Schaeffer does for the PT relating the reduced decrees of freedom in the parameter space.
%
%%
...
In particular it is thought that catch data can induce "contrast" in index data
so as to better inform $r$. It is not well understood how contrast may factor 
into biases induced by model misspecification. To investigate this a variety of
...
to yield fast and reliable inference \shortcite{deyoreo_integrating_2012}. %deyoreo_integrating_nodate}. %({\color{red} Cite Maria DeYorio pdf}).
...
Figure(\ref{expCatchT45}) demonstrates this more realistic, while still somewhat idealic, %an idealised and realistic, while still simplified, 
...
additional 15 time epochs, so that slower growing stocks may observe more data 
...
%%
%The meta-modeling approach presented here indicates that the Schaefer model 
...
toward only modestly misspecified models as seen in Figure (\ref{expT60BmFm}).   
...
curves which could be use instead of current two parameter curves.
...
instructive for a variety of non-compensatory production behaviors, it is possible 
that under different modeling assumptions, for example compensatory BH production 
or age structured models, different bias patterns will emerge. Extending this work 
to be able to make claims in those settings is necessary for developing more 
generally extensible claims. % and my proposal for this extension can be seen in Section(\ref{})
...
%
\clearpage
\section{Extensions to other productions functions}

%
\clearpage
\section{Catch Interpolation}
...
%
%\begin{figure}[h!]
\includegraphics[width=.85\textwidth]{../../../theses/nick/spline/dataPlots505.png}
%\caption{ \label{expCatchT45Fmsy}
%}
%\end{figure}
\end{minipage}
...
%%
%\begin{figure}[h!]
\includegraphics[width=.85\textwidth]{../../../theses/nick/spline/intCurves505.png}
%\caption{ \label{expCatchT45Bmsy}
%}
\end{minipage}
...
\begin{align}
y(t^*) &= k(t^*)-k(\tau^*) ~~~ \tau^*=\lceil t^* \rceil-1 \\
k(t^*) &= \int_0^{t^*} x(t) dt\\
x(t) &= \beta_0 + \sum_{j=1}^{T-1} \beta_j (t-\tau_j) \mathds{1}(t>\tau_j)
\end{align}

\begin{align}
y(\tau_i) &= \beta_0(\tau_i-\tau_{i-1}) + \sum_{j=1}^{i-1} \beta_j \left[ \left(\frac{\tau_i^2}{2}-\tau_j\tau_i\right) - \left(\frac{\tau_{i-1}^2}{2}-\tau_j\tau_{i-1}\right)\right] + \epsilon_i \\
...
form $p(q, \sigma^2, \bm{\theta}|I) = N(\log(q)| \sigma^2, \bm{\theta}, I) IG(\sigma^2|\bm{\theta}, I) p(\bm{\theta}|I)$ \shortcite{deyoreo_integrating_2012} %({\color{red} Cite Maria DeYorio pdf??}).
