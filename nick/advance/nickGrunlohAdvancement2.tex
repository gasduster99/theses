...
The GP meta-model is explicate about trade-offs between RPs %to the surface bias tradeoffs to the surface to 
...
production function as a special case to form the Schaefer model. The Pella-Tomlinson 
production function is parameterized so that $\bm{\theta} = [r, K, \gamma]$ 
...
Figure(\ref{expCatchT45}) demonstrates this more realistic, while still somewhat idyllic, %an idealised and realistic, while still simplified, 
...
\begin{align}
\frac{dB}{dt} = \frac{\alpha B}{(1+\beta B)^\gamma} - (M+F)B \label{schunte}
\end{align}
...
%
\begin{align}
\frac{dB}{dt} = \frac{\alpha B}{(1+\beta B)} - (M+F)B \label{bh}
\end{align}
...
\begin{itemize}
	...
	\item Recruitment and Maturity occur at age $k$ (knifedge)
	\item No fishing or natural mortality below $k$, but above everyone is the same
\end{itemize}
...
\frac{dB}{dt} &= \overbrace{w(k)R(B(t-k))}^\text{Recruitment Biomass} + \overbrace{\mu \left[w_\infty N(t)-B(t)\right]}^\text{Net Growth} - \overbrace{(M+F(t))B(t)}^\text{Mortality}\\
\frac{dN}{dt} &= R(B(t-k)) - (M+F(t))N(t)\\
R(B) &= \frac{\alpha B}{(1+\beta B)^\gamma}
\end{align}

\noindent
Nuisance Parameters: $q$, $\sigma^2$\\
Recruitment Parameters: $\alpha$, $\beta$, $\gamma$\\
Growth Parameters: $\mu$, $w_\infty$\\
$\mu$: Metabolism, $w_\infty$: Maximum weight?\\
...
While methods for quantifying catch uncertainty have been developed \shortcite{shelton_estimating_2012}, 
the effect of uncertainty in catch has not been incorperated in stock assessment models \shortcite{methot_stock_2013}.    

%
The theoretical use of splines for smoothly and accruately approximating the 
...
incorperating elements of catch uncertainty into single species stock 
assessment models. 
...
naive discretizations can cause, forward Eurler discreteization of dynamics is 
largly implemented for convience for use with discretely sampled data such as 
catch. For inference (when parameters are not known in advanced) a continuous 
handling of biomass dynamics largly avoids the numerous numerical issues 
brought about by naive time stepping. However, a continuous approach requires 
tactful methodology for moving catch estimates from the current yearly 
estimates to the interpolated instantaneous quantities needed for a continuous 
...
the integral of landed biomass from $\tau_{i-1}$ to $\tau_i$. The instaneous 
...
on regular unit epoch intervals, and the change of basis time integrals are 
collapsed, the following observation model for $y(\tau_i)$ arrives naturally. 
%\vspace*{-1cm}
...
linear in $\beta$; infact this approach results in another spline model on $y$, 
albeit represented in a different basis. Secondly, by explicatly modeling $x(t)$ 
with a spline it gives direct interpretibility of the $\beta$s as slopes of 
the instantaneous catch.  
...
inorder to model jittery observations, while the black model fit, which assumes 
...
\\(\emph{left})\\ Namibian Hake yearly catch \mbox{observations} plotted along with \mbox{predictive model fits.} (\emph{right}) \mbox{Posterior estimates} of instantanious catch with and without assumed \mbox{uncertatinty} in observed catch. 
}
\end{minipage}
\end{figure}

%
The model fit attained under the assumtion of complete certainty in catch 
...
Given the parsamonious bottom-up derivation of this model, these observations 
suggests that the assumption of certainty among catches is poor.
% the source of the observed defective.


%
I propose that this modeling paradigm presents a useful tool for propogating 
catch uncertainty into the dynamics of stock assessment models. Firstly this 
method provides a flexible, and interpretible, method for smoothing catch 
...
explored. Additionally, the effect of accounting for catch uncertainty with this 
...
\gantttitle{S}{2}
\gantttitle{S}{2}\\
\ganttgroup{Project 2}{5}{10} \\
\ganttgroup{Project 3}{9}{14} 
\end{ganttchart}
\end{center}
...
\bibliography{./gpBias.bib,./spline.bib}
%\thispagestyle{next}
