...
##for(i in 1:length(datFiles)){
	...
	#
        optAns = fit$optimize(cpue,
                c('kappa', 'WW'),
                lower   = c(kappa/5, WW/5),
                upper   = c(2*kappa, 2*WW),      #log(getBeta(100, -1, M, P0))
                gaBoost = list(run=10, parallel=T, popSize=10^3),#10^4),
                persistFor = 5,
                fitQ    = F
        )
	...
	#
        optAns = fit$optimize(cpue,
                c('lsdo', 'lalpha', 'lbeta', 'kappa', 'WW'),
                lower   = c(log(0.001), log(M+kappa), -10, kappa/5, WW/5),
                upper   = c(log(1), log(100), -2, 2*kappa, 2*WW),      #log(getBeta(100, -1, M, P0))
                gaBoost = list(run=10, parallel=T, popSize=10^3),#10^4),
                persistFor = 5,
                fitQ    = F
        )
