...
#
listFile = "./expModestGrow.csv" #"./expGrowFitters.csv"
#safely append
app = F
if( file.exists(listFile) ){
        #
        isGo = readline(sprintf("Overwrite %s?\nCtrl-C to Escape, Enter to Overwrite, or A to Append: ", listFile))
        #Append
        if( toupper(isGo)=="A" ){ app=T 
        #Overwrite      
        }else{ app=F }
} 
...
for(i in 1:nrow(l)){ #nrow(out$ll)){
	...
	app = T
        ...
        ##fit$plotMean( add=T, col='red') #, main=sprintf("%1.2f, %1.2f", l[i,1], l[i,2]), ylim=c(0,fit$B0) ) #out$ll[
