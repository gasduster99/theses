...
#
mod = "ExpT45N150A15" #"ExpT45N150K1" # #"FlatT30N150A15K0.1" #"ExpT45N150A15" # "ExpT45N150Wide" #"ExpT45N150A15K0.1" #"ExpT45N150K1" #
...
##
#registerDoParallel(n^2)
#opts = list(preschedule=F)
##
#foreach(i=1:nrow(l), .options.multicore = opts) %dopar% {
for(i in 1:nrow(l)){
	#
	if( !i%in%tops ){ next }
	#
	...
	#
	TT = length(dat$time)
	dat$lsdo = log(116.0256)
	#
	cpue = rlnorm(TT, dat$lq+log(dat$B), exp(dat$lsdo))
	
	#gsub("datGen", "fitThree", datFiles[i])
        fileDatFit = sprintf('%sfitThree_lsdo%s_%s', place, round(dat$lsdo), rownames(l)[i]) 
        if( file.exists(fileDatFit) ){ next }
	#
	ww = dat$WW*(1-exp(-dat$kappa*dat$a0))
	lbetaBH = log(getBeta(10000, M, kappa, ww, dat$WW, exp(dat$lalpha), -1))
	datFit = ddModel$new( derivs=der,
        	N0Funk=function(lalpha, lbeta, gamma, M, WW, kappa, a0){#virgin numbers
                        #
                        alpha = exp(lalpha)
                        beta  = exp(lbeta)
                ...
                time=dat$time, catch=dat$catch, a0=dat$a0, M=dat$M, WW=dat$WW, kappa=dat$kappa,#constants
                lalpha=dat$lalpha, lbeta=fit$lbeta, gamma=fit$gamma, #lalpha=dat$lalpha, lbeta=dat$lbeta, gamma=dat$gamma,          #parameters
                lq=log(0.00049), lsdo=log(0.01160256),                  #nuisance parameters
                xi=dat$xi, zeta=dat$zeta, cpue=cpue                          #other incidentals to carry
        )
	datFit$iterate(odeMethod)
	
	print(fileDatFit)
	
	#
	check = sprintf('%scheckFitThree_lsdo%s_%s', place, round(dat$lsdo), rownames(l)[i])
	if( !file.exists(check) ){	
		optAnsG = datFit$optimize(cpue,
                        c('lsdo', 'gamma'),
                        lower   = c(log(0.001), fit$gamma-1.5),
                        upper   = c(log(1), fit$gamma+1.5),
		...
		#
		optAnsBG = datFit$optimize(cpue,
                        c('lsdo', 'lbeta', 'gamma'),
                        lower   = c(log(0.001), -10, datFit$gamma-1.5),
                        upper   = c(log(1), -2, datFit$gamma+1.5),
                        gaBoost = list(run=10, parallel=T, popSize=10^3),
	        	#cov     = T,
                        fitQ    = F
                )
		...
		datFit = readRDS(check)
		
		#log(dat$M+dat$kappa)
		optAnsAG = datFit$optimize(cpue,
                        c('lsdo', 'lalpha', 'gamma'),
                        lower   = c(log(0.001), log(dat$alpha/2), datFit$gamma-1.5),
                        upper   = c(log(1), log(dat$alpha*2), datFit$gamma+1.5),
                        gaBoost = list(run=10, parallel=T, popSize=10^3),
	        	#cov     = F,
                        fitQ    = F
                )
		...
		#
		optAnsABG = datFit$optimize(cpue,
                        c('lsdo', 'lalpha', 'lbeta', 'gamma'),
                        lower   = c(log(0.001), log(dat$alpha/2), -10, datFit$gamma-1.5), #min(fit$gamma,dat$gamma)/2),
                        upper   = c(log(1), log(dat$alpha*2), -2, datFit$gamma+1.5),
                        gaBoost = list(run=10, parallel=T, popSize=10^3),
	        	cov     = T,
                        fitQ    = F
                )
        ...
        #convenience
        datFit$alpha = exp(datFit$lalpha)
        datFit$beta  = exp(datFit$lbeta)
        datFit$sdo   = exp(datFit$lsdo)
        datFit$q     = exp(datFit$lq)
        #save
        datFit$save( fileDatFit )
...
#
png(sprintf('indexFitGrid%s_lsdo%s.png', mod, round(dat$lsdo)), width=2000, height=2000)
layout(lay)
for(i in 1:nrow(l)){ #nrow(out$ll)){
	...
	#
	fdWho = sprintf('%sfitThree_%s', place, rownames(l)[i])
	if( !file.exists(fdWho) ){
                #writeLines(sprintf('\nSKIP Xi: %s, Zeta: %s\n', xiSims[j], zetaSims[i]))
                next
        }
	datFit = readRDS(fdWho)
	
	#
	par(mar=c(1,1,1,0))
	dat$plotMean() #main=sprintf("%1.2f, %1.2f", l[i,1], l[i,2]), ylim=c(0,dat$B0) )
	datFit$plotMean( add=T, col='red') #, main=sprintf("%1.2f, %1.2f", l[i,1], l[i,2]), ylim=c(0,fit$B0) ) #out$ll[i,1], out$ll[i,2]), ylim=c(0,out$mod[[i]]$B0) )
	datFit$plotBand( col='red' )
	points(datFit$cpue)
}
dev.off()
