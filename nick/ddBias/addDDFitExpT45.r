...
#for(i in (1:length(datFiles))){
	...
	fitBHKA = ddModel$new( derivs=der,
		...
		xi=fitBH$xi, zeta=fitBH$zeta               	#other incidentals to carry along
	)
	...
	#
        optAns = fitBHKA$optimize(cpue,
                c('kappa', 'aS'),
                lower   = c(kappa/20, aS/20),
                upper   = c(20*kappa, 20*aS),      #log(getBeta(100, -1, M, P0))
	...
	#
        optAns = fitBHKA$optimize(cpue,
                ...
                lower   = c(log(0.001), log(M+0.1), -10, kappa/20, aS/20),
                upper   = c(log(1), log(10), -2, 20*kappa, 20*aS),      #log(getBeta(100, -1, M, P0))
                gaBoost = list(run=10, parallel=T, popSize=10^3),#10^4),
                persistFor = 5,
                fitQ    = F
        )
	#get hessian if possible
        tryCatch({
                optAns = fitBHKA$optimize(cpue,
                        c('lsdo', 'lalpha', 'lbeta', 'kappa', 'aS'),
                        lower   = c(log(0.001), log(M+0.1), -10, kappa/20, aS/20),
                        upper   = c(log(1), log(10), -2, 20*kappa, 20*aS),
                        cov     = T,
                        fitQ    = F
                )
        }, error=function(err){
	...
	fit3KA = ddModel$new( derivs=der,
		...
		xi=fit3$xi, zeta=fit3$zeta               	#other incidentals to carry along
	)
