...
% accounting for all naturally occurring %no mirgration the population is assumed closed. 
...
From a management perspective a major goal of modelling is to accurately infer
...
indicates a value at steady state.  The steady state biomass is a 
function of $F$.
%; we will see a specific example of this in
%\mbox{Section (\ref{ptRef}).} 
MSY is found by optimizing $\bar Y(F)$ with respect to
...
however under the under the assumption of a two parameter production function the model
will be structurally unable to capture the full theoretical range of RPs 
...
via calibration modeling techniques. These studies indicate improtant factors that
...
using intentionally misspecified two the parameter logistic or BH production models.
%
...
%
Indices of abundance are simulated from the three parameter PT production model
broadly over the space of $F^*$ and $\frac{B^*}{\bar B(0)}$ values. These PT 
data are then fit with a misspecified two parameter Schaefer model 
so as to observe the effect of productivity parameter model misspecification 
upon RP inference.
...
%model are obtained, and PT index of abundance data can be   for the purpose of generating index of abundence 
...
%
It is known that the behavior of catch can effect inference on the productivity %biological 
...
is not well understood how contrast may factor into inferential failure induced by model %biases induced by model 
...
$B^*$. For constant $\frac{F(t)}{F^*}$ biomass %the Schaefer model 
%has an analytical 
...
$B(t)$.%, on the condition that the form of $\frac{dB}{dt}$ is at least Lipschitz continu
...
explicatly here. % as opposed to  inclusion of natural mortality, $M$. 
...
%%
%In the Schaeffer and PT models
%$M$ is modeled expliciatly here  and $F$ are instantaneous rates of natural and fishing 
%mortality respectively. Unlinke the PT model described above 

%with Schnute production simulates a
%wide varienty of possible population productivety behaviours and provides an 
...
%
For the purposed of simulation, it is not neccessary to completely know 
%either of 
...
(\ref{FmsyS}) and (\ref{B0S}) are solved for $\alpha$ and $\beta$ respectively, to produce % as a function of RPs 
the partial mapping $\big(F_{MSY}, B_0\big) \mapsto \big(\alpha(\cdot, \gamma), ~\beta(\cdot, \cdot, \gamma)\big)$ 
in terms of RPs and $\gamma$. 
% yeilds a partial mapping from 
...
and $\beta$ for a given $\gamma$. % if only $\gamma$ were further isolated
...
%prohibitivly expensive, and solutions are often still unreliable. 
%
...
\subsubsection{Latin Hypercube Sampling}
	...
	\begin{itemize}
		...
		%\item[\checkmark] approximate LHS
		%\item[\checkmark] $\frac{B_{MSY}}{B_0} (\gamma) = \zeta(\gamma)$ pic, $\alpha(\gamma)$, $\beta(\alpha, \gamma)$ 
	\end{itemize}
...
%Letting $T$ be the CDF of the modeled distribtuion of $\gamma$, the values of $\mu$, $\sigma$, and $\nu$ are chosen by minimizing the $L^2$ distance between $T(\gamma)$ and $\zeta(\gamma)$.
...
%\end{itemize}
%%\end{wrapfigure}
%\caption{Sudo-code proceedure for LHS sampling in $\mathcal{F}\times\mathcal{B}$.}
%\label{sudoCode}
%\end{minipage}
%\end{figure}
%\noindent
...
axis of so as to normalizing distances to a common scale. 
...
simulation. Furthermore, since the design space has a nonlinear constraint, the 
calculation of $x_a$ is further truncated based on a convex hull defined by the 
existing samples in the overall design.
...
%
In summary, an initial $X_{n} = LHS_{n}(R_{full})$ design is computed based on 
an overall simulated region of RPs $R_{full}$. The maximin augmenting point, 
$x_a$, is computed at a maximin distance of $d_a$ from the existing samples. 
...
%Letting $S$ be the square that circumscribes the circle certered at $\bm{x_a}$ 
...
%Given the tempermental relationship between RPs and productivity parameters in 
...
%estimate of two productivity pa-
%225 rameters under the restricted production model. Each of the fitted productivty parameter
%226 estimates are then modeled using independent instances of the following model in Eq (28).
...
%proposes extensions to the immediatly following work which generalize results 
%to a broader set of dynamics. %production functions, as well as proposes an analyze mechanisms of 
%%model failure in the 
%%context of simple age structured dynamics. 
...
Firstly, the population grows through a production function, $P(B)$. Production 
in this setting is defined as the net biomass increase due to all reproduction 
and maturation processes accounting for all naturally occurring %no mirgration the population is assumed closed. 
