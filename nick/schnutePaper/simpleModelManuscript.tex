...
$B_0$, $\frac{B_{MSY}}{B_0}$, the above system can fully specify $\theta$ if only 
...
$\zeta$ are unstable. Numerically inverting $\zeta$ quickly becomes 
...
along grids of size $n$ in each dimension of the design space. By intersecting 
the grids of each dimension, $n^m$ cells are produced, from which a total 
$n$ samples are taken. Crutially only one sample is taken from a given element 
