...
will be refered to as the ``Schaefer line''. Thus simulated data that are 
generated along the Schaefer line will be the only data that are not 
...
misspecification of the Schaeffer model becomes worse and worse.
...
production model, 
%by transforming  %(in this case the Schaeffer model when fitting PT data. 
by transforming metamodel predictions via Eq. (\ref{ptRP}), metamodeled predictions 
are obtained for Schaeffer RPs. By further subtracting the true data generating 
PT RPs from the predicted Schaeffer RPs at each point in RP space a pattern of 
...
\end{minipage}
\begin{minipage}[h!]{0.44\textwidth}
\includegraphics[width=\textwidth]{../ptNew/directionalBiasSubPTExpT45.png}
\end{minipage}
...
(\ref{arrowsPT}) is that since the Schaefer model must estimate RPs on the Schaefer line, 
the metamodel arrows indicate the mapping of RPs that is created by inferring RPs under 
...
$\frac{B_{MSY}}{B_0}$ direction must simply map vertically onto the Schaefer line.
...
the closest possible point along the Schaefer line that Schaefer model inference %under a Schaefer model 
...
Any deviation from this minimal bias pattern neccissarily to be due to added bias in $F_{MSY}$.
...
for data generated below the Schaefer line and overestimated above the Schaffer line.
...
{\color{orange} By including more contrast $F_{MSY}$ bias in this low $\frac{B_{MSY}}{B_0}$, 
high $F_{MSY}$ region can be further reduce.}


%that is induced by the catch series that defines the amount of contrast present in the simulated indicies.
%On the $left$ 
%and the bias in the $\frac{B_{MSY}}{B_0}$ direction is simply the distance   
...
%\item Point out the geometry of Schaeffer RPs, thus $\frac{B_{MSY}}{B_0}$ must 
...
%225 rameters under the restricted production model. Each of the fitted productivity parameter
