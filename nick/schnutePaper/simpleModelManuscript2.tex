...
to focus analysis on signal in the simulation results. Metamodeling occurs on 
the inferred productivity parameters of the restricted production model (in this 
case the Schaeffer model when fitting PT data. 




 Additionally the metamodel 
generalizes the patterns seen in Figure (\ref{flatProd}) across the entire range of RPs. 

the   not neccessarily produce a 
...
$F_{MSY}$ as observed in other studies (cite). 
\item Histogram of high/low contrast bias observations.
\item adding information (High contrast) reduces $F_{MSY}$ bias and thus moves 
the mapping closer to minimum possible distance.  

\item 

\item summary of $\sigma$ across models to show model breakdown.
\end{itemize}
