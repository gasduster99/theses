...
\title{\vspace*{-2.25cm}Metamodeling for Bias Estimation of Biological Reference Points} % Under the Schaefer Model}
...
for their pervasive use in modern fisheries \shortcite{mangel_perspective_2013,punt_extending_2019}.
...
Similarly the Beverton-Holt \shortcite[BH]{beverton_dynamics_1957}
and Ricker \shortcite{ricker_stock_1954} curves are also two parameter
...
1 respectively, and the Ricker model is a limiting case as $\gamma\rightarrow0$. %\shortcite{schnute_general_1985}.
...
%
%By working with the expressions $\frac{F_{MSY}}{M}$, $B_0$, $\frac{B_{MSY}}{B_0}$
...
$B_0$, and $\frac{B_{MSY}}{B_0}$ %$B_{MSY}$ % 
...
%$\frac{F_{MSY}}{M}\in(0.25, 4)$, and letting $\mathcal{B}$ be an 
%equally spaced grid of size $n$ for $\frac{B_{MSY}}{B_0} \in (0.15, 0.7)$. 
%$\mathcal{F}$
%
...
%%quantities) given $B_0$, $M$, and $F_{MSY}$. %$\mathcal{F}_i$.
%\label{sudoGrid}}
%\end{figure}
%
%%
...
%design elements, $(\frac{F_{MSY}}{M}, \zeta^*)\Leftrightarrow(\alpha^*, \beta^*, \gamma^*)$,
%have been computed for all $i\in[1,...,n]$.


%With $T(\gamma|\hat\mu, \hat\sigma, \hat\nu)$ fitted for a specific $F_{MSY}$ 
...
%	\item[$\forall i$:]
%	\item[1)] Draw $\frac{F_{MSY}}{M} \sim Unif(\mathcal{F}_i)$.
%	\item[2)] Compute $[\hat\mu, \hat\sigma, \hat\nu]$ given $F_{MSY}$ and $M$.
...
%	\item[7)] Compute $\alpha^* = \alpha(\gamma^*, F_{MSY}, M)$. 
%	\item[8)] Compute $\beta^* = \beta(\alpha^*, \gamma^*, M, B_0)$.	
%	\item[9)] \mbox{Save $(\frac{F_{MSY}}{M}, \zeta^*)\Leftrightarrow(\alpha^*, \beta^*, \gamma^*)$ in $\mathcal{F}_i\times\mathcal{B}_j$.} 
...
%Since the scales of $\frac{B_{MSY}}{B_0}$ and $\frac{F_{MSY}}{M}$ differ, the 
%distance function 
...
%\begin{equation*}
%\underbrace{\left(F_{MSY}, \frac{B_{MSY}}{\bar B(0)}\right)}_{\text{PT Truth}} {\text{GP}\atop{\mapsto\atop~}} \underbrace{\left(\hat F_{MSY}, \frac{\hat B_{MSY}}{\bar B(0)}\right)}_{\text{Schaefer Estimate}}
%\end{equation*}
...
ultimatly brings fishing into equilibrium at $F_{MSY}$. %   around the turn of the century, followed by a rapid
...
using the lsode integration %method 
...
%since the Shaefer line is simply a constant at $\frac{B_{MSY}}{B_0}=0.5$, 
...
0.699 and 0.201 respectively. %Bias is shown for the Schaefer models estimation of $F_{MSY}$ as a function of contrast.
%A comparison of the true PT production function (in black) and the estimated logistic curve (in red)
%with 95\% CI shown. The examples shown represent the four corners of maximum model misspecification
%in the simulated RP-space. Observed biomasses are plotted in the rug plots below the curves.
%%Fitted logistic production function verses true PT production function. 
...
%
Figure (\ref{conTest}) demonstrates how bias in $F_{MSY}$ estimation %under the Scaaffer model 
...
model of fishing.
%,  in may be By including more contrast $F_{MSY}$ bias  in this low $\frac{B_{MSY}}{B_0}$, 
%high $F_{MSY}$ region can be further reduce.}
...
%%smooths and pulls information together.
%
%%\item Point out the geometry of Schaefer RPs, thus $\frac{B_{MSY}}{B_0}$ must 
%%simply map vertically. 
%%\item Bias in $\frac{B_{MSY}}{B_0}$ estimation entirely defined by the degree 
%%of model mispecification.
%%\item mapping directly vertically is a minimum distance mapping given the 
%%constrained space.
%
%%\item Any biases in $F_{MSY}$ moves this mapping further from the mapping with 
%%minimum possible distance.
%\item The low contrast simulation indeed demonstrates biased estimation of 
%$F_{MSY}$ as observed in other studies {\color{red}(cite)}. 
%
%%%\item adding information (High contrast) reduces $F_{MSY}$ bias and thus moves 
%%%the mapping closer to minimum possible distance.   
%%\item {\color{orange}bias in $F_{MSY}$ at a point in the lower right corner as a function of ``contrast''.}
%
%%%At some point
...
%%	\end{itemize}
%
%%\item Histogram of high/low contrast $F_{MSY}$ bias observations??: I don't think this is neccessary unless really pushed by someone
%\end{itemize}
...
%a uniform marginal in $\frac{B_{MSY}}{B_0}$. A uniform marginal in $\frac{F_{MSY}}{M}$ can 
%be enforced directly. %Algorithm (1) also ensures that the resulting design respects a latin 
%square. 

%The only apprixmate result is in the uniformity of the $\frac{B_{MSY}}{B_0}$ 
%marginal which is excelent.
...
%RPs are limited to the curve $\frac{B_{MSY}}{B_0}=\frac{1}{F_{MSY}/M+2}$.
%Again, for brevity define the BH set as the set of RPs defined by this limited space, 
%i.e. the curve $\left\{\left(\frac{B_{MSY}}{B_0}, \frac{F_{MSY}}{M}\right) \;\middle|\; \frac{B_{MSY}}{B_0}=\frac{1}{F_{MSY}/M+2}\right\}$.
%The farther away from this set that Schnute data are simulated, the worse the 
%BH model is misspecified for those data. 
%
%
%\begin{itemize}
%\item demonstrate that my space filling design does in fact result in a uniform $\frac{B_{MSY}}{B_0}$ margin.
...
%This has the effect of allowing the bias in $\frac{B_{MSY}}{B_0}$ and $F_{MSY}$ 
...
%\begin{itemize}
%\item Bias present in high contrast $\frac{F_{MSY}}{M}$.
%\item While bias is present in $F_{MSY}$ the BH mapping is similar to the Schaefer simulation in that the overall mapping
%is very near a minimal disance mapping in RP space.
%\item bias present in $\frac{F_{MSY}}{M}$
%\end{itemize}
...
\vspace{-1cm}
\caption{
%Relative bias in $F_{MSY}$ in the high contrast simulation setting. The BH set is also shown in black. 
...
%information in the data to produce reasonable estimates of $\frac{F_{MSY}}{M}$ 
...
%may fail by completely disregaurding one RP so the sake of preserving whatever 
...
%
%This suggests that in the absense of information
...
%Outside of the safe regime inference in the absense of informative information. BH inference maps to 
%this common memeber of the Schaeffer-Like, Ricker-Like and BH-Like regimes. 
%This is represents catastrophic inferential failture.
%
%
...
%that are resonably well modeled by a BH model. 
%
...
%%\item ``safe'' regime similar to the high contrast minimal distance mapping for mild model misspcificaiton.
...
%\end{minipage}
%\caption{Joint bias direction of RP infernce in the low contrast simulation setting.
%%sfor $\left(\frac{F_{MSY}}{M}\right.$, $\left.\frac{B_{MSY}}{B_0}\right)$ 
%%estimates under the misspecified BH Model. 
%The intensity of color represents the excess bias relative 
%to the shortest possible mapping. 
%%The high constrast relative fishing setting is 
...
These studys appreciate the role of contrast for estimating growth rates, 
...
of productivity estimation within the prodcution model setting presented. 
...
%%
%\begin{itemize}
%\item \shortcite{lee_can_2012} Generalizability via random sample of fisheries, LHS RP explicatly does this.
%\item multiple starting locations and convergence defined as positive definate hessian.
%\end{itemize}
...
%
%NOTE: BITCH ABOUT NOT BEING ABLE TO PRODUCE A PERFECTLY INFOMATIVE TIME SERIES
...
broad spectrum of relavant fishing behaviors, it is still limiting in the amount of information 
...
inference fails, but themethods presented here tell the most complete picture 
yet, with explicit control of the degree model misspecification, contrast, and 
a simulation design that allows for uniform representive data generation 
...
mapping of RPs represents a best-case sceneario where the total bias of RPs, 
...
bias in one of $\frac{B_{MSY}}{B_0}$ or $F_{MSY}$, there will neccessarily be 
bias in the other RP. However under the Schaefer model, since the RP set is a 
constant in $\frac{B_{MSY}}{B_0}$, bias in $F_{MSY}$ is not adulterated in the 
same way by bias in $\frac{B_{MSY}}{B_0}$ estimation. While models with 
constant RPs, such as the logistic model $\frac{B_{MSY}}{B_0}=\frac{1}{2}$ or 
the fox model $\frac{B_{MSY}}{B_0}=\frac{1}{e}$, are extremely limited, they 
...
the severe implication of management recommedations potentially leading to 
...
likely to make conservative errors in its estimates of $F_{MSY}$ and $\frac{B_{MSY}}{B_0}$. 
The one notable exception to the conservatism of the BH model stands for data 
generated in the cushing-like regime of Schnute RPs. In this regiem the BH 
...
For any population with $\frac{B_{MSY}}{B_0}<0.5$, $\frac{B_{MSY}}{B_0}$ will be overestimated. 
When the population comes from the regime where $\frac{B_{MSY}}{B_0}>0.5$, $\frac{B_{MSY}}{B_0}$ 
will be under estimated, but $F_{MSY}$ is likely to be overestimated depending on the degree of 
...
choosing a BH model of productivity. %Ultimatly accuracy of RP estimation, 
%without structural RP biases, %and accurate assement of uncertainty is a better state 
...
%conjecture that with perfectly informed models mapping would be orthoganal
\begin{itemize}
...
%\begin{align}
%%\frac{B_{MSY}}{B_0} &= \frac{1-\left(\frac{M+F_{MSY}}{\alpha}\right)^\gamma}{ 1-\left(\frac{M}{\alpha}\right)^\gamma } \nonumber\\
%0 &= \frac{1}{\gamma} - \left(\frac{1}{\gamma} + \frac{F_{MSY}}{F_{MSY}+M}\right)\left(\frac{F_{MSY}+M}{\alpha}\right)^\gamma  \label{system}\\
