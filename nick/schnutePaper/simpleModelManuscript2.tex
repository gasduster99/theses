...
resemble the minimal distance mapping seen in Figure (\ref{contrastTrio}, \emph{top-right}). %under the high contrast simulation setting. 
...
generated for Cushing-like or BH-like production. While bias of the RPs can still become concerning %(as large as $X$ in the worst case) within this region, the region can be considered safe, for the BH model, in the 
sense that even for extremly poorly informed (i.e. low contrast) data RP estimation 
recovers the minimal distance mapping.
...
\vspace{-1cm}
\caption{
Yield curves for data generated with $\frac{F_{MSY}}{M}=3.48$ and $\frac{B^*}{\bar B(0)}=0.47$. 
...
at the interface between BH-Like and Ricker-Like regimes, as seen in 
