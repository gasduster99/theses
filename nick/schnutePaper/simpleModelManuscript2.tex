...
%	\item \shortcite{stein_large_1987} large smaple props. 
%	\item Monotinicity: Latin hypercube sampling yields a smaller variance of the sample mean than simple random sampling
...
%
At its core, a metamodel is simply a model of a mapping of inputs to outputs.
A metamodel provides a specific representation of an ontology of the modeled 
mapping. %the mapping %that can be seen to form
By modeling the mapping as a statistical model, and simulating examples of the 
mapping, the inferential infrastructure of the statistical model is used to 
empirically learn an effective ontology of the system. %, based on simulated examples of the mapping. 
...
%
In this setting, the aim of metamodeling is to study how well RPs are infered 
...
observe particular instances of how well RPs are infered at the given 
misspecification of the two parameter production model relative to the true three parameter 
production model. By gathering all of the simulated instances of how RPs are 
infered (under the two parameter model), %under misspecified two parameter models
...
misspecified two parameter production model. The metamodel is essentually a surrogate 
...
%with points closely related in the index set resulting in correlated effects in
%the model. 
Typically correlation functions are specifed so that points closely related in 
space result in correlated effects in the model. In this setting the inputs to 
the GP metamodel are the space of reference points with define the simulated 3 
parameter production models. 
...
% as well asestimation uncertainty (via the inverted Fisher information).
...
production model to indcies of abundance generated under three parameter productivity.
%BH model 
