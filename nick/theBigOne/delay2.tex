...
without requiring the use of heavily parameterized ASMs. %e.g. \cite{black rockfish assessment}.
...
equations describe %exactly models 
the population dynamics of the total exploitable biomass $B(t)$ and number of indivuduals $N(t)$ through time.
%%
%\begin{align}
...
these parameters only describe the net processes of larval production %, and maturation into the population, 
...
to catestrophically fail in RP estimation. % model failure when modeled with a BH model, such that stocks above each 
...
%of the $w(a_s; \kappa)=c$ level curve, 
...
(i.e. evaluation of the yeild curve) %yield 
...
%as well as the growth and maturity parameters $\kappa$ and $a_s$.
...
%
The addition of individual growth and maturity dynamics to the previous chapters, %to the RP analysais in Chapter (\ref{schnuteChapter}), 
via the DDM, further reiterate the general patterns observed in Chapter (\ref{schnuteChapter}). 
...
restricted model rather than stock biology. Furthermore, the added dynamics of the DDM 
demonstrates that the BH model can become very brittle for slower growing stocks; 
the added flexibility of three parameter models can relieve these issues.
...
minor BH model misspecification with intact shortest distance RP estimation, and a region of %for relatively minor BH misspecification, and a region of 
...
regiems of individual growth (i.e. $corr(a_s, \kappa)<0$ \cite{denney_lifehistory_2002}),
the analysis of these dynamics demonstrate how individual growth 
...
%as individual growth becomes a more important dynamic. 
...
recruitment can certainly contribute to better estimates of RPs.
Three parameter models of recruitment can be estimated to allow better estimates 
of RPs, even when growth dynamics are included in the model, or three-parameter 
models may be used by analyses similar to that in Appendix (\ref{proxyMath}) to 
increase the flexibiliy of recruitment by use of three parameter models.
...
that is availble in limited datasets. Furthermore, future investigation 
should be considered for how the use of three-parameter recruitment models may 
be capable of supplementing the inference of individual growth and maturity 
parameters in addition to the external analysis of age data. 
...
here an extrordinarily general, and compact, model for analysis of a wide 
...
wide range of individual growth and maturity dynamics from simple SPMs, 
lag-only models similar to \cite{dick_depletion-based_2011}, ranging up to 
simple ASM-like growth dynamics. While not all RPs can be represented 
analytically, many can be under many common models. The analytical results 
presented here, and the methods presented for navigating RPs, in this 
wide varity of different modeling situations opens the door to the many future 
uses in stock assessment. 
