...
Cohorts are governed by a system of similar ODEs such that 
...
the population dynamics of the total exploitable biomass $B(t)$ and number of individuals $N(t)$ through time.
%%
%\begin{align}
...
equations as opposed to the simple ODEs that arise in the production model
setting.
...
expressions look somewhat unwieldy\footnote{complicated by growth parameters}, although analytical solutions can still be
...
inspired framework that uses the GP metamodel to propagate estimate
...
parameters and RPs. Under the BH model $\bm{\phi}$ is not well informed and 
typically would be estimated externally to the DDM for data-limited stocks.
%
...
restricted model rather than stock biology. Furthermore, the added individual 
...
exacerbates model misspecification by dampening the information that individual growth contributed to biomass dynamics.
