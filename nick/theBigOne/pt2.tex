...
%, at each design location of the simulation, 
...
%For the Schaefer model the MLE of $r$ is transformed to $\log(F^*)=\log(\frac{r}{2})$ %and $\log(B_0)$
%%the productivity parameters are transformed (via the expressions developed in Section (\ref{ptRef}) with $\gamma=2$) 
%%to $\log(F^*)$ %and $\log(B_0)$
...
%so that the modeled quantity, $\log(F^*)$, is in $\mathbb{R}$.
%
%of RPs, via 
...
collecting the fitted MLEs for $\log(F^*)$, and let %=\log\left(\frac{r}{2}\right)$, and let  
%(transformed so that the free RP under the Schaefer model is in $\mathbb{R}$), and let
$\bm{\omega}$ be a vector of estimates of the estimator variances (via the
inverted Fisher information) at each $\textbf{y}$.
%
The transformed RP estimates are then modeled using the following GP metamodel. %in Eq (\ref{GPModel}). 
%}
...
%{\color{blue}
%Recall that metamodeling occurs on the scale of $\log(F^*)$ under the Schaefer model. 
...
RPs are obtained.
 %via the expressions in Eq. (\ref{ptRP}).
%Recall that metamodeling
...
included. %The metamodeled quantity $\log(B_0)$ does not appear anywhere else in this analysis. 
%. due not contribute to the overall analysis that appears in this document. 
%}
%{\color{red} a sentence about modeling $\log(B_0)$ to look at $B^*$ and/or $MSY$}
...
setting the scale of dynamics. %determining the scale biomass 
%Note that $K$ appears in the expression of $\bar B$ as a leading multiplier.
In low contrast settings when $B_t\approx\bar B(\bm{\theta})$ for most $t$, 
the product $qB_t(\bm{\theta})$ is essentially $q\bar B(\bm{\theta})$. In turn, % which in turn 
$q\bar B(\bm{\theta})$ can be written as $qK(1-F/r)$ to reveal the product $qK$. 
%exposes the leading $K$ from the $\bar B$ expression, and reveals the product $qK$. 
Since the likelihood will take the same value when swapping the values of $K$ 
and $q$ in these cases, only their product may be identified. %rather than the parameters.
...
%to essentally   
%to inform $r$ well enough so that the correlation structure between $r$ and $K$ 
%can 
% on the parameters or  
% demonstrating
%and $r$ is very poorly informed
%, only the product $qB_t(\bm{\theta})$ may be identified but individually $q$ and 
%$\bar B(\bm{\theta})$ may exchange values to result in the same product. Thus 
%the values $q$ and $\bar B(\bm{\theta})$ are not indiviually identifiable in 
%these cases.

%issue. %makes this issue when the 
...
and consequently also overestimates $F^*$. %for data generated above the Schaefer line. 
...
%%biomasses decay exponentially from $K$ to $B^*$ in each case. In particular,
%%notice how observations only exist where the PT biomass is greater than $B^*$.
%%Due to the leaning of the true PT curves, and the symmetry of the 
%logistic
%parabola, the logistic curve only observes information about its slope at the
%origin from data observed on the right portion of the PT curves. 
%The top two
...
\end{minipage}
\begin{minipage}[h!]{0.44\textwidth}
\includegraphics[width=\textwidth]{../edits/directionalBiasSubPTTitleFlatT30nF1.png}%../ptNew/directionalBiasSubPTFlatT30.png}
\end{minipage}
