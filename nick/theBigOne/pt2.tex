...
%
Historically the Schaefer model is a canonical model in fisheries. % is the Schaefer model. The
...
\end{align}
$r$ is a parameter controlling the maximum rate of population growth (biomass production) %per capita growth rate %of surplus biomass production 
...
%
\subsection{\color{red}Gaussian Process Metamodel\label{gpmm}}
%

{\color{red}
Clear up $\sigma$ notation CRISPY\\
Indicate uses outside of \cite{kennedy_bayesian_2001}
}
...
%(i.e. Schaefer:[$log(r)$, $log(K)$], BH:[$log(\alpha)$, $log(\beta)$]).
These quantities are then modelled as GP models over the broader RP space.
...
%
Each of the transformed parameter estimates are then modeled using
...
%
$\hat y(\textbf{x})$ is the metamodel's predicted value of the modeled $\log(RP)$ estimates %MLEs
...
Finally, $\textbf{r(x)}$ is a vector-valued function of correlation function 
evaluations for thepredictive location $\textbf{x}$ against all observations 
