...
%
%\subsection{The Yeild Curve \& The Rate of Replacement}
...
\verb|dede| function of the R package \verb|deSolve| \shortcite{soetaert_solving_2010}. 
...
R package GA \shortcite{scrucca_ga_2013, scrucca_extensions_2017} is used to 
...
$\frac{F_{MSY}}{M}$ is dramatically underestimated, it is very natural to ask 
...
forming hypotheses to identify this misspecified RP region. The single clearest 
feature to identify are cases where $\frac{F_{MSY}}{M}$ is estimated well below the 
minimum values simulated. In the follow section this idea is formally stated in a 
hypothesis testing structure that uses the GP metamodel as a surrogate for the 
sampling distribution of $\frac{F_{MSY}}{M}$ under the misspecified BH model. 
This allows for a rejection threshold to be derived in terms of the GP 
predictive structures that defines a classifier for identifying BH inference 
break points broadly over RP space.

%
For simplicity in outlining the hypothesis testing framework, let $\theta$ be the
metamodeled population parameter, corresponding with $\frac{F_{MSY}}{M}$, under 
the two parameter BH model and let $\theta_{min}$ its minimum simulated value.
% of the corresponding quantity. %
In this parlance, the idea of dramatic under estimation of $\theta$ may be formally 
represented by the following hypothesis.
%
\begin{equation}
H_0~:~\theta\ge\theta_{min}~~~~~H_a~:~\theta<\theta_{min} \label{hyp}
\end{equation}
%%
%These hypotheses effectively simplify the separation of the two major RP regiems 
%based on the metamodeled behavior of $\frac{F_{MSY}}{M}$ relative to the 
%region of RPs simulated. 

%%
%Given the behavior observed in Section (\ref{schnuteLowResults})
%BH model has the biggest effect on $\frac{F_{MSY}}{M}$ is the single biggest impact RP

%Thus observing simulations where  

%
For evaluating the hypothesis test, the metamodel prediction serves as a 
descriptor of RP estimation using kriging to provide both a measure of mean 
behavior as well as propogating estimate uncertainty via the kriging predictive 
variance. The metamodeled quantity is then predicted by $N(\hat y(\textbf{x}), \hat \sigma^2(\textbf{x}))$,
%$\hat y$ (a measure of mean behavior), but also propogastes estimate uncertainty.
% ed uncertainty serves to 
where $\hat y(\textbf{x})$ is as previously described in Eq. (\ref{gpYHat}) and 
$\hat \sigma^2(\textbf{x})$ propogates estimate uncertainty via the kriging 
predictive variance given by,%
...
%
For evaluating the hypotheses given in Eq. (\ref{hyp}) %, $\hat y(\textbf{x})$ is considered as a test statistic, and 
$H_0$ is then rejected when $\hat y(\textbf{x})$ is small in the following sense 
%
\begin{align}
%\hat y(\textbf{x})
R &= \{ \textbf{x}:~\hat y(\textbf{x})<C \}. \label{rejC}
%\alpha_0 &= Pr( \hat y(\textbf{x})<c )
\end{align}

%
To calibrate exacly how small the constant $C$ should be the false positve 
rate, $\alpha_0$, is considered. For evaluating $R$ under the null hypothesis, 
the domain of the metamodel is limited along the axis of $\theta$ as, 
$\textbf{x}_{min} = [\theta_{min}, \textbf{x}_{(-\theta)}]$. Thinking of 
$\hat y(\textbf{x})$ as an estimator of $\theta$, via kriging, the following 
test statistic distibution follows naturally under the null,
%
\begin{equation}
\frac{\hat y(\textbf{x}_{min}) - \theta_{min}}{\hat \sigma(\textbf{x}_{min})} \sim N(0,1).
\end{equation}
%
This stratforwardly implies $\alpha_0 = \Phi \left( \frac{C - \theta_{min}}{\hat \sigma(\textbf{x}_{min})} \right)$, 
and rearranging for $C$ gives $C=\theta_{min}+\Phi^{-1}(\alpha_0)\hat \sigma(\textbf{x}_{min})$. 
Replacing $C$ into Eq. (\ref{rejC}), the rejection region can be made specific as
%
\begin{equation}
%\hat y(\textbf{x})
R = \{ \textbf{x}:~\hat y(\textbf{x})<\theta_{min}+\Phi^{-1}(\alpha_0)\hat \sigma(\textbf{x}_{min}) \}.
\end{equation}

%
Other hypotheses may be formed using a similar structure (or added to this 
structure) to fine tune the rejection threshold, although a point of 
deminishing returns will quickly set in for describing the catestophic model 
failure under low contrast BH inference. By simple inspection of Figure (\ref{bhLowArrows}), 
clearly the under estimation of $\frac{F_{MSY}}{M}$ is a very powerful 
classifer for describing BH inference trends. 


%%
%\begin{itemize}
%	\item Refer to previous section for failure behaviour (form hypothesis out of this)
%	\item adressing this hypothesis requires a complete handling of metamodel uncertainty 	
%	\item state Rejection region philosophy
%	\item calculate $c$
%	\begin{itemize}
%		\item note that clustering problem reduces to discerning behaviour at $\textbf{x}_{min}$ 
%			since $\theta\in\textbf{x}$ and $\theta_{min}$ ia the case of interest under the null
%		\item $\hat \sigma^2(\textbf{x})$ only needs to be comuted for $\textbf{x}_{min}$
%	\end{itemize}
%	\item state complete Rejection region
%	%\item layout math
%	%\item GP Mean: \verb|XStar%*%gpFit$beta + SIGStar%*%strongInv(SIG)%*%(y-X%*%gpFit$beta)|
%	%\item GP Variance: SIGStarStar + SIGStar%*%strongInv(SIG)%*%t(SIGStar) + tg|
%	%\item \verb|seTry = sqrt(gpPredictVar(lFXSSmall, lFXSSmall[,2:3], tg=0, lFFit))|
%	%\item \verb|metaLowerZetaThresh0.5 = (exp(lFPred[1,]-1.96*seTry)/M)[diffs0.5==min(diffs0.5)]|
%\end{itemize}



%%The kriging preditive distribution forms a complete repeated sampling theory across RP space as 
%\begin{equation}
%	\text{Kriging Prediction:~} N(\hat y(\textbf{x}), \hat \sigma^2(\textbf{x})).
%\end{equation}

%%Under The Null:
%\begin{align}
%%\textbf{x}_{min} &= [\theta_{min}, \textbf{x}_{(-\theta)}]\\
%\frac{\hat y(\textbf{x}_{min}) - \theta_{min}}{\hat \sigma(\textbf{x}_{min})} &\sim N(0,1) \\
%\alpha_0 &= \Phi \left( \frac{C - \theta_{min}}{\hat \sigma(\textbf{x}_{min})} \right) \\
%c&=\theta_{min}+\Phi^{-1}(\alpha_0)\hat \sigma(\textbf{x}_{min})
%\end{align}
...
%The Logistic model on the other hand ct some what similarly
%\begin{itemize}
%\item RP curve triptic (BH, Ricker, Logistic) each at the three examples
%\end{itemize}

%and steeply increasing recruitment for large biomass 
%populations interacting with the shock of the onset of fishing from the initially 
%unfished population.

%opposing slow indivdual growth represents 
...
%reasonably well modeled by a BH model. While bias of the RPs can still become concerningly large
%
...
%\begin{itemize}
%\item Reiterate from simple case
%\end{itemize}
...
\includegraphics[width=0.48\textwidth]{../ddBias/directionalBiasDDSubFlatT45N150A0-1AS4K0.2N56.png} %94 %directionalBiasDDFlatT45N150A0-1AS4K0.2N28.png}
\vspace{-0.75cm}
\caption{
...
%can also be seen in the effect they 
%have on RPs. Reducing 
%Furthremore, by observation of the 
%constrained RP sets in Figure (\ref{rpTriptic}), this simulated setting 
%produces a large deviation from the simple (no growth) production model;
%emphasizes the effect of growth on RPs.
%% of the constrained RP set under the 
%%simple (no growth) production model, as seen in Figure (\ref{rpTriptic}).
%
%{\color{red} another paragraph}
...
%In the high contrast simulation setting Figure (\ref{moderateGrowth}; $left$),
%the RP mappings again demonstrate a nearly identical minimal distance mapping of
%RPs onto the constrained BH RP set. 
%
%In the low contrast setting Figure (\ref{moderateGrowth}; $right$)
%a very similar two regiem pattern of RP inference is observed, however the
%location of the break between these regiems appears at lower values of
%$\frac{B^*}{\bar B(0)}$. In this moderate growth setting the break point
%occures around values of $\frac{B^*}{\bar B(0)}$ just below 0.4. as opposed
...
minimum simulated RP becomes more difficult as those values $\hat y(x)$ from $\theta_{min}$ has maximum overlap at $\theta_{min}$

For large $\frac{F_{MSY}}{M}$ the minimum distance mapping results in $\hat y(x)$ well above the minimum simulated RP
but for small $\frac{F_{MSY}}{M}$ even the minimum distance mapping is may be close to the rejection threshold.

The second axis of sensativity is between individual growth simulations. The no growth  

\begin{itemize}
\item $\alpha_0\approx0.16$$\alpha_0\in[0.5, 0.0225]$ shows
\item clustering is more uncertain for small $F_{MSY}/M$ since differentiating $\hat y(x)$ from $\theta_{min}$ has maximum overlap at $\theta_{min}$.
\item no growth is more certain than emphtic growth, since estimates under the emphatic growth setting are more uncertain leading to more overlap between infence classes.
...
%%PROD
%Indeed in the high contrast setting, Figure (3.6, left) shows how the BH model induces
%1193 the same pattern of bias as seen in Chapter (2). There is bias in both RPs (in accordance
%1194 with the B∗ = 1 RP-set) so as to produce a nearly minimal distance mapping of B ̄ (0) F ∗ /M +2
%1195 RPs onto the constrained BH set of RPs. Similarly, in the low contrast setting, Figure (3.6,
%1196 right) again shows the same two regiems pattern of RP inference. 
%
%Firstly, there is a region
%1197 of relatively small model misspecification where the minimal distance mapping is preserved.
%1198 Secondly, as model misspecification becomes greater (around the Ricker set) F∗ begins to M
%1199 be sharply underestimated. Above this break point in RP estimation inference appears to
%1200 be driven to the trivial RP F ∗ = 0, B∗ = 0.5) that is shared in common amoung all of the
%
%%MODERATE
%The RP mappings seen in Figure (3.7) show very similar RP mappings to that of the
%1219 simple production model, with the biggest differences occuring around the location of the
%1220 break point where the low contrast model begins to dramatically underestimate F ∗ . In the M
%1221 high contrast simulation setting Figure (3.7; left), the RP mappings again demonstrate a
%1222 nearly identical minimal distance mapping of RPs onto the constrained BH RP set. In the
%
%1223 low contrast setting Figure (3.7; right) a very similar two regiem pattern of RP inference is
%1224 observed, however the location of the break between these regiems appears at lower values
%1225 of B∗ . In this moderate growth setting the break point occures around values of B∗ just B ̄ (0) B ̄ (0)
%1226 below 0.4. as opposed to the simple production model where the break point occurs at B∗ B ̄ (0)
%1227 just above 0.4.
%
%%EMPHATIC
%Despite the emphaticly growth driven dynamics in this setting, the RP mappings seen
%1240 in Figure (3.8) obviously bare a huge resemblance to the previously seen RP mappings.
%1241 Again the biggest differences in the RP mappings occur around the location of the break
%1242 point where the low contrast model begins to dramatically underestimate F ∗ . In this low M
%1243 contrast setting the break point in RP estimation occures around values of B∗ well below B ̄ (0)
%1244 0.4 with the behaviour extending as far down as B∗ = 0.3. This regiem shift occurs well B ̄ (0)
%1245 below that of the Ricker set, as initially observed in the production model setting. This
%1246 reduced range of acceptible RP inference indicates that under increasingly dramatic growth
%the model misspecification issue of the BH model becomes an increasingly brittle assumption with respect ot RPs.
%
%Interestingly this pattern only follows for the low contrast setting. In the 
%high contrast setting inference returns to a pattern resmbleing the minimal 
%distance mapping onto BH RP set. Further pointing to the importance of 
%contrast for informing these models.%

%The ocillatory regiem introduces some unique inferential challenges
%Considering RP estimation in the ocillatory regiem 

%%\begin{figure}[h!]
%\begin{wrapfigure}{r}{0.50\textwidth}
%\includegraphics[width=0.49\textwidth]{../ddBias/directionalBiasDDSubExpT45N300A0-1AS10K0.1.png}
%\caption{}\label{ocillationArrow}
%\end{wrapfigure}
...
%%\includegraphics[width=0.5\textwidth]{plots/derisoSrr.png}
%%\begin{minipage}[h!]{0.64\textwidth}
%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridFlatT45N300A0-1AS0.1K10.png}
%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridFlatT45N300A0-1AS10K0.1.png}
%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridExpT45N300A0-1AS0.1K10.png}
%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridExpT45N300AS10K0.1.png}
%%\end{minipage}
%%\begin{minipage}[h!]{0.3\textwidth}
%\vspace{-1.5cm}
%%\hspace*{-1cm}
...
%%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridFlatT45N300A0-1AS0.1K10.png}
%%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridFlatT45N300A0-1AS10K0.1.png}
%\includegraphics[width=0.54\textwidth]{../ddBias/directionalBiasDDSubExpT45N300AS0.1K10.png} %directionalBiasDDExpT45N300AS0.1K10.png
%\includegraphics[width=0.54\textwidth]{../ddBias/directionalBiasDDSubExpT45N300A0-1AS10K0.1.png} %directionalBiasDDExpT45N300A0-1AS10K0.1.png}
%%\end{minipage}
%%\begin{minipage}[h!]{0.3\textwidth}
%\vspace{-1.5cm}
%%\hspace*{-1cm}
...
%%\begin{minipage}[h!]{0.64\textwidth}
%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridFlatT45N300A0-1AS0.1K10.png}
%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridKAFlatT45N300A0-1AS10K0.1SDORAND.png}
%\includegraphics[width=0.54\textwidth]{../ddBias/indexGridKAExpT45N300AS0.1K10.png}
...
%At equilibrium 
%$\frac{dB}{dt}=\frac{dN}{dt}=0$, and thus %moving forward  equationsSince both equations are in equilibrium equate 
%\begin{align}
%w(a_s)R(B) + \kappa \left[w_\infty N-B\right] - MB = R(B) - MN.
%\end{align}
%
%Making use of the equlibrium value $\bar{N} = R(B)/M$ to rewrite $N$ in terms of $B$, %and collecting like terms gives, %by making use of $\bar{N} = R(B)/M$,  
\begin{align}
...
%\frac{dN}{dt} = R(B)-MN.
%\end{equation}
%Setting $\frac{dN}{dt}=0$ gives the numbers equilibrium equation, $\bar{N} = R(B;\theta)/M$.
%%
%Simultaneously, consider the biomass equation in the absence of fishing,
%\begin{equation}
%\frac{dB}{dt} = w(a_s)R(B;\theta) + \kappa w_\infty N - \kappa B - MB. %\label{bEq}
%\end{equation}
%Setting $\frac{dB}{dt}=0$, and solving for $N$ further implies,
%%
%\begin{equation}
%N = \frac{1}{\kappa w_\infty}\left[(M+\kappa)B + w(a_s)R(B;\theta)\right] \label{nBarB}.
%\end{equation} 
%
%%
%\begin{align}
%&\frac{dB}{dt} = w(a_s)R(B) + \kappa \left[w_\infty N-B\right] - (M+F)B \\
%&\frac{dN}{dt} = R(B) - (M+F)N 
%\end{align}
%
%
%
%%
%\begin{align}
%R(B;\theta) - MN = w(a_s)R(B) + \kappa \left[w_\infty N-B\right] - MB \\
%\end{align}
%
%\begin{align}
%&N = \frac{1}{\kappa w_\infty}\left[(M+\kappa)B - w(a_s)R(B;\theta)\right] \\
%&N = R(B;\theta)/M 
%\end{align}
%
%\begin{equation}
...
%\end{equation}
%
%With numbers at equilibrium, now further restrict consider the equilibrium biomass equation. 
%
%\begin{align}
%0 = w(a_s)R(B;\theta) + \kappa \left[\frac{w_\infty}{M}R(B;\theta)-B\right] - MB \label{bEq}
%\end{align}
%
%\begin{equation}
%0 = \frac{R(B)}{M}
...
%
%% 
%In the simple production model $M$
%
%%
%Maximum recruitment: $\alpha$
%$F=\alpha$ pushes recruitment to 0.
%$\alpha$ below replacement line means fishing pushes population to extinction.
