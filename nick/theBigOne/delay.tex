...
%
Figure (\ref{delayTriptic}) demonstrates a range of behaviors that the Schnute 
delay model can display. The three special cases of $\gamma=-1$ (BH), $\gamma\to0$ 
(Ricker), and $\gamma=1$ (Logistic) recruitment are shown with varying configurations 
...
configuration represents a RP set in the deep blue regiem, thus this case represents 
...
%
Notice under the extreme $a_s=10$ and $\kappa=0.1$ setting, biomass %the Ricker and Logistic models 
comes into equilibrium at $B_{MSY}$ as oscillating curves. This effect is most 
noticable in the Logistic model here due to its steeply right leaning yeild curve 
interacting with the large lag in selectivity as the sudden onset of fishing 
produces a shock that oscillates over the steepest regions of the yeild curve. 
One may exagerate the oscillations of a BH model similarly, in this heavily lagged setting, by 
shocking the population over the steepest portion of its yeild curve (a sudden 
release in fishing applied to a heavily fished population at low equilibrium biomass). 

%
\begin{figure}[h!]
\includegraphics[width=\textwidth]{../ddBias/rpTriptic.png}
\caption{
Biomass dynamics of BH ($left$), Ricker ($center$), and Logistic ($right$)
delay differential models in the low contrast simulation setting. In all cases
$\alpha=1.2$ and $\beta$ is chosen so that each model shares the same
...
%
\clearpage
\subsection{Ocillatory Growth Influence}
