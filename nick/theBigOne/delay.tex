...
$Z=F+M$ (representing total mortality rate) has been made to produce a more 
compact expression.
...
%
In the simple production model the BH constrained RP is fixed at $\frac{1}{x+2}$; 
under the BH delay model the growth parameters $a_s$ and $\kappa$ slightly 
...
Schnute delay model. However, just as for the simple production model, 
...
%
Similar to the Schnute production model setting a partial mapping for
$\big(F^*, B_0\big) \mapsto \big(\alpha(\cdot, \gamma), ~\beta(\cdot, \cdot, \gamma)\big)$
can be derived analytically in terms of RPs and $\gamma$. The substition $Z^*=F^*+M$ is 
made where $F^*$ and $M$ appear together to produce a more compact expression.
...
%
While conceputally this proceedure is similar to the Schnute production model, 
the analytical expression is substantially more complex, with growth parameters 
appearing explicitly in the expression.
