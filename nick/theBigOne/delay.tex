...
\shortciteA{von_bertalanffy_quantitative_1938} in length with age. To model 
...
to have zero weight, thus allowing fish of age zero to have positive weight. 
Rather than taking a sigmoidally increasing function, VB growth directly in 
weight results in an monotonically inceasing curve that asymptotes with a strictly
decreasing growth rate with age. 
...
to a common heavyside step function with age. That is to say, before age $a_s$ 
the population is assumed not to experience any mortality whatsoever, but all 
fish older then $a_s$ experience the same rate of natural mortaility. Simulaneously 
all fish older than $a_s$ are equally vulnerable to fishing, although fishing 
effort may vary from through time. 
...
growth parameters appearing in Eq (\ref{bEq}) to convert numbers to biomass and 
account for the growth of biomass with age. 
...
size and growth associated with changes in recruitment.    
...
flexible family of functions may model this relationship. For the sake of evaluating the assumption 
of BH recruitment the simulation setting below is derived for the delay model under the assumption 
of a three parameter Schnute recruitment as follows.
%
\begin{align}
R(B;[\alpha, \beta, \gamma]) = \alpha B(t-a_s)(1-\beta\gamma B(t-a_s))^{\frac{1}{\gamma}} \label{srr}
\end{align}
%
The parameters $\alpha$, $\beta$, and $\gamma$, function similarly in this setting as previously 
described in Section (\ref{}). The value of $\gamma$ controls the behavior of recruitment 
from the special case of BH recruitment at $\gamma=-1$, through the Ricker model as 
$\gamma\rightarrow0$, to Logistic recruitment when $\gamma=1$.
%
%The structure of the Schnute function here is similar to that of the previously described, with $\gamma$ . 
%
The delay model's assumption of knife edge maturity introduces a time delay between spawning biomass and the number of recruits produced. 
...
\begin{itemize}
	...
	\item The BH and Logistic production functions arise when $\gamma$ is fixed to -1 or
1 respectively. 
	...
	\item time delay
	\item interpretation of recruitment (larval production, recruitment) [growth external] vs. production (larval production, recruitment, growth)
\end{itemize}
...
%
\begin{align}
\bar{B}(F) = \frac{1}{\beta\gamma} \left( 1 - \Big(\frac{(F + M) (F + M + \kappa)}{\alpha w(a_s)(F + M + \kr)}\Big)^\gamma\right)
\end{align}

%
\begin{align}
\bar{N}(F) = \frac{\alpha\bar{B}(F)(1-\beta\gamma\bar{B}(F))^{1/\gamma}}{F+M}
\end{align}
...
\scriptsize
\begin{align}
\alpha & = \left[ \oneA + \twoA \left( 1 + \thrA \right) \right]^{\frac{1}{\gamma}}\\
