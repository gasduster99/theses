...
to view the BH model of recruitment as a default \cite{methot_stock_2013, dick_stock_2023}. %, black}. 
...
as a production function to model adult biomass production as, % consider the % By solving this differential equation they derive 
%
\begin{equation}
...
%is fixed at $10000$ as in Chapter (\ref{ptChapter})
...
respectively. Marginally this corresponds to $\frac{\alpha}{M}\in(2, 50)$ with the most typical values lying below 17. %based on the marginal 95\% highest density interval. 
...
$\frac{F^*}{M}$) before surplus production becomes %. Under the Schnute model values of $\zeta(\gamma)$ below $\zeta_{min}$ result in 
negative\cite{myers_maximum_1999, punt_extending_2019}. % productivity % defined by the relationship between Schnute productivity and linear natural mortality.
...
%three-parameter location-scale family Student's $t$
...
%
To demonstrate how convergence of the metamodel is identified %below results are shown from the 
Figure (\ref{edit2}) shows the convergence process using the results from Section (\ref{ptTrends})
as an example. The  metamodel trends shown in the first column %row 
...
design, the pattern \mbox{originally shown in Figure (\ref{arrowsPT}) emerges.} %appears (with 100 samples).}
...
shown in Figure (\ref{arrowsPT}). In each case, %the first column shows the 
the first column demonstrates the unconverged metamodels with an initial 50 sample design. %(demonstrating the unconverged metamodels), and 
...
%unconverged metamodel (with 50 dessamples), each subsequent collumn shows the metamodel after an additional 50 
%samples  demonstrates an  the metamodel is  The first three collumns show progressively more dense designs
}
\end{sidewaysfigure} 
...
%shown in Figure (\ref{arrowsPT}). In each case, %the first column shows the 
%the first row demonstrates the unconverged metamodels with an initial 50 sample design. %(demonstrating the unconverged metamodels), and 
...
\clearpage
\noindent
%the metamodel trends are stable and do not %change with addition samples.
...
mappings described by the simulated models.%The interpolation of the most denbse desijgn is shown to demonstrate that the 
...
remaining the same.
%%=\log(\sqrt{\alpha M}-M)$ (as derived from Eq. (\ref{FmsyS}) with $\gamma=-1$  
...
%Recall that metamodeling occurs on the scale of $\log(F^*)$ under the BH model.
...
%GP to  While the mapping 
%By further subtracting the true data generating PT RPs from these predicted Schaefer RPs
...
misspecification of the BH model, can be visualized. %as will be seen in Figure (\ref{arrowsPT}).
%}
...
$\frac{B^*}{B_0}$. %Under the BH model $\frac{B^*}{B_0}=\frac{1}{\frac{F^*}{M}+2}$, 
...
\vspace{-1cm}
\caption{
Yield curves for data generated with $\frac{F^*}{M}=1.97$ and $\frac{B^*}{\bar B(0)}=0.46$. %$\frac{F^*}{M}=3.48$ and $\frac{B^*}{\bar B(0)}=0.48$.
...
A similar observation was made under the Schaefer model and the mechanism driving this phenomenon %mechanism
is established in Section (\ref{flat}).
%As similar phenomenon
%This is similar as was 
%observed under the Schaefer model. A mechanism driving this phenomenon under the Schaefer 
%model is established in Section (\ref{flat}). 
...
misspecification. When not enough contrast is present in the data to %is too little to
...
%to focus on fitting to the overall scale of biomass.
...
%In all cases when model misspecification is removed
As seen in all of the metamodeled trend figures (\ref{contrastTrio} and \ref{bhLowArrows}), 
even with weakly informative data, when data are generated from RPs immediately around the BH set %(or nearly BH) data  RPs
...
estimates as best as possible within the BH set. %In a sense 
...
$\frac{B^*}{\bar B(0)}$ (as opposed to the Schaefer model where $\frac{B^*}{\bar B(0)}=\frac{1}{2}$ for all $\frac{F^*}{M}$. %when this is not true for the Schaefer model 
%makes contrast a crucial factor for estimating $\frac{B^*}{\bar B(0)}$ in the BH model. 
%%
...
a nearly best-case scenario. %(at least within the {\color{blue}safe region}).
...
%some of the least resilant stocks to overfishing
%
%This has  potentially leading 
%to overfishing for more severe management consequences for those populations.
...
%overfishing, while underestimation leads to overly cautious management.  
%In this sense, when the true model is not known the geometry of the BH set together
%with the metamodeled bias trends makes BH estimates naturally cautious %conservative
%for the less vulnerable stocks modeled here. For the less vulnerable stocks (stocks above the BH set) the 
%BH model is likely to underestimate $\frac{F^*}{M}$. %both ___ and $\frac{B^*}{B_0}$.
...
%vulnerable stocks, and yet $\frac{F^*}{M}$ tends to be overestimated by the BH model in this regime. 
%This leads to the regrettable situation where the most vulnerable stocks are subject to 
%subtle but systematic overestimation of $\frac{F^*}{M}$, while the less vulnerable 
%stocks tend 
%
%the least vulnerable stocks 

%This leads to the regrettable
...
%headwind against many vulnerable stocks. 
...
%three-parameter models we can release these underlying structural limitations \cite{mangel_perspective_2013}. 
...
%to solve a number of structural 
%inconsistencies {\color{red}\cite{nickEJ}} in fisheries management (see 
%Appendix (\ref{proxyMath}) for details). 
