...
as an example. The  metamodels shown in the first collumn are made with an 
intentionally sparse intial design (50 samples) to examplify an unconverged 
metamodel. As more samples are added to the design, the metamodeled pattern 
orignally shown in Figure (\ref{arrowsPT}) appears. %(around 100 samples). 
...
the first column demonstrates the unconverged metamodels with an inital 50 sample design. %(demonstrating the unconverged metamodels), and 
The second and third collumns demonstrate the metamodel trends with progressively more dense designs. 
The final collumn shows the a nearest neighbour interpolation of the raw data of the most dense design.
...
and do not change with addition densification of the underlying design. A 
nearest neighbour interpolation of the raw data from the densest design is 
