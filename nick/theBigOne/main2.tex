...
limits RP estimates to the horzontal line at $\frac{B^*}{\bar B(0)}=\frac{1}{2}$. 
...
reparameterized directly in terms of their RPs, allowing for straight-forward inference.  
Figure (\ref{exRP}) shows these RP estimates in relation to the respective two/three-parameter RP spaces under each model. %in the broader three-parameter RP space. 
...
tight uncertainty bounds in $F^*$ and absolute certainty that $\frac{B^*}{\bar B(0)}=\frac{1}{2}$. 
The fact that PT can model $\frac{B^*}{\bar B(0)}$ more generally than the 
Schaefer model lets the PT model reflect the shortcomings of these data 
to provide a more honest representation of RP estimates with wider uncertainty bounds. %in the form of .
...
this issue arrises when estimating both quantities in low contrast simulations. %, as described in Section (\ref{modelFit}). 
