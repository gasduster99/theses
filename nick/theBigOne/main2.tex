...
that areassumed to remove biomass linearly with biomass. Above, $Z(t)$, is 
...
also included other linear terms in $Z(t)$. Commonly the rate of ``natural 
...
As data permits this general structure is often expanded to account for age, weight, 
and/or length classes \cite{schnute_general_1987, fournier_length-based_1987} in 
the population in models called {\it Age structured models} (ASM). ASMs are currently considered best
practice in ``data-rich'' settings when there is considered enough data to 
establish length/age relationships and selectivity \cite{maunder_review_2013, cope_stock_2023}. 
Modeling age structure in the population allows for individual growth and maturity 
to be modeled as cohorts that age through time. Despite this added complexity, 
at their core, ASMs rely on similar dynamics as seen in Eq. (\ref{ode}), 
however in ASMs the above ODE is expanded to a system of interrelated ODEs for 
each age class.  

%
{\it Delay-differential models} (DDM) can provide an intermediate modeling 
infrastructure between SPMs and ASMs. By simplifying the model assumptions 
around growth and maturity, simple ASMs may be represented exactly 
by DDMs \cite{deriso_harvesting_1980}. Furthermore, SPMs can be represented as a limiting case of DDM 
growth and matruity parameters. This makes DDMs ideal for ``data-limited'' 
assessment settings. Despite the useful bridge that DDMs provide, their use 
in stock assessments has been sparing due to the complexity of deriving key 
manaagment quantities under these models \cite{dick_depletion-based_2011, munyandorero_analytical_2023}.
%Often the complexdue to a lack of methods . 
...
%
\chapter{\color{red}Synthesis of Conclusions and Future Works}
\clearpage
...
{\color{red} nonstationarity in populations may move the true RPs of existing populkations into these more vulnerable areas. by sticking to a BH model we will have a hard time understanding those changes as they occur. and a more flexible model would be flexible in noticing those changes.}  
...
%
\chapter{A Scheafer Model with Explicite $M$ \label{mApp}}
