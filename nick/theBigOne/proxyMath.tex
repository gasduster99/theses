...
%cite Clark (1991, 1993, 2002) \cite{clark_groundfish_1991, clark_f_2002} Mace (1994) \cite{mace_relationships_1994} to derive, Punt \cite{punt_how_2012} in use
%}
...
\end{equation}
%
By setting $x$ appropriately the hope is that $F_{SPR_x}$ is a easily calculable proxy %measurable
for $F^*$. Calculating $F_{SPR_x}$ is relatively more straightforward than $F^*$ 
because it is independent of the %a helpful proxy because it is independent of the 
...
thought of as a proxy for $\frac{B_{MSY}}{B_0}$. Enforcing both %measureable
...
and $\gamma$ that defines an infinite %family of Schnute curves that respect the proxy values $x$ and $y$.
...
%MSY RP curve, along side Schnute MSY RP curves with $\gamma$ chosen to match the management pr
\end{wrapfigure}
%
...
values generalize the BH model in a way that results in familiar production models %curves 
