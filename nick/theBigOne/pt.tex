...
given level of spawning output. \cite{mangel_perspective_2013} %Mangel et al. (2013) 
...
limit the theoretical domain of RPs \cite{mangel_perspective_2013}. The
...
\begin{figure}[h!]
	...
	\caption{\label{hakeData}
        \textit{left}: An index of abundance data, catch per unit effort (CPUE), for Namibian Hake from 1965 to 1987 \protect\cite{hilborn_ecological_1997}.
...
Similarly the two parameter Cushing \cite{cushing_dependence_1971}, Beverton-Holt \cite{beverton_dynamics_1957} %\cite[BH]{beverton_dynamics_1957} %
...
%RPs \cite{mangel_perspective_2013}.
...
are not fully understood or described. \cite{lee_can_2012} provides some
evidence that estimation of productivity parameters %, and thus RPs via \cite{mangel_perspective_2013},
are dependent on biomass contrast as well as model specification. % as well as contrast in data %lacking contrast in biomass. 
\cite{conn_when_2010} comes to similar conclusions %about steepness estimation
...
%1 respectively. The Ricker model is a limiting case as $\gamma\rightarrow0$. %\cite{schnute_general_1985}.
...
%
%%
%Similarly to \citeA{schnute_analytical_1998}, expressions %solving expressions 
...
%%	\item \cite{gramacy_surrogates_2020} The goal is to guarantee a certain degree of spread in the design, 
%%	while otherwise enjoying the properties of a random uniform sample.
%%	\item \cite{devon_lin_latin_2015}  Construction and properties.
%%	\item \cite{stein_large_1987} large sample props. 
%%	\item Monotonicity: Latin hypercube sampling yields a smaller variance of the sample mean than simple random sampling
%%	\item \cite{owen_central_1992} CLT
%%\end{itemize}
%
%%
...
%%\item maximin designs are often concentrated in the corners (edges) of the design region
%%
%%\item \cite{johnson_minimax_1990}
...
%%\item \cite{morris_exploratory_1995}
...
%This has the advantage of definitionally filling holes in the design, %\cite{johnson_minimax_1990}, %in a way that has been %shown to have asymtoptically optimal characteristics for Gaussian process models \cite{johnson_minimax_1990}
...
%via the Livermore Solver \cite[lsode]{radhakrishnan_description_1993}, and catch 
...
%to yield fast and reliable inference \cite{walters_calculation_1994}. %deyoreo_integrating_2012}. %deyoreo_integrating_nodate}. %({\color{red} Cite Maria DeYorio pdf}).
...
\cite[p.2]{wanner_solving_1996} describe stiffness in the
...
%%
%\begin{itemize}
%\item \cite{lee_can_2012} Generalizability via random sample of fisheries, LHS RP explicitly does this.
...
\cite{punt_extending_2019} %(Mangel et al., 2013). Punt and Cope (2019) 
...
\item {\color{blue}show a schnute fit to data? \cite{yeakel_generalized_2015} Prior} % and how it fixes issues.}
...
%release these structural limitations \cite{mangel_perspective_2013}. 
%\citeA{punt_extending_2019} considers a suite of possible three parameter
...
%	\caption{\label{hakeData}
%	\textit{left}: An observed series of index of abundance data for Namibian Hake from 1965 to 1987 \protect\cite{hilborn_ecological_1997}.
%	\textit{right}: The associated catch data for Namibian Hake over the same time period.
%	}
%\end{figure}
%
...
%%
%Secondly, the population decreases as biomass is removed due to catch, $C(t)$. 
%While catches (aka yields) are observable quantities \cite{pearson_documentation_1997}, %sen_sampling_1984,  
%the model assumes that catch is proportional to biomass with the 
%proportionality constant representing the fishing rate, $F(t)$, so that $C(t)=F(t)B(t)$. 
...
%Schaefer model is formed by choosing $P$ to be logistic growth \cite{mangel_theoretical_2006} %({\color{red} cite}) 
...
%pervasive use in modern fisheries \cite{mangel_perspective_2013,punt_extending_2019}.
%
%%%
...
%of the production function are the Beverton-Holt \cite[BH]{beverton_dynamics_1957} 
%and Ricker \cite{ricker_stock_1954} curves. All of these two parameter 
...
%On the one hand, \citeA{maunder_is_2003} argues that the Schaefer model is 
...
%(PT) model \cite{pella_generalized_1969} should replace the Schaefer model to avoid biased parameter 
%estimates. On the other hand, while \citeA{prager_reply_2003} appreciates 
...
%%The argument of \citeA{maunder_is_2003} suggests that the two parameter 
...
%is still under debate because the particular mechanisms and behavior (direction and magnitude) %over relevant RPs %and mechanisms %behavior 
%of these biases for key management quantities %(and the mechanisms of inducing these biases) 
%are not fully understood or described. \citeA{lee_can_2012} provides some 
%evidence that estimation of productivity parameters, and thus RPs via \cite{mangel_perspective_2013}, 
%are dependent on biomass contrast as well as model specification. % as well as contrast in data %lacking contrast in biomass. 
%\citeA{conn_when_2010} comes to similar conclusions %about steepness estimation
...
%model. Finally a Gaussian Process (GP) metamodel \cite{gramacy_surrogates_2020} is constructed for exploration 
...
%%theoretical space of RPs \cite{mangel_perspective_2013}.
%
%
%
%%{\color{gray}
...
%parameters \cite{hilborn_quantitative_1992}. % {\color{red} cite walters}). 
%In particular it is thought that catch can induce "contrast" in index data
...
%model in a way similar to the nugget effect \cite{gramacy_cases_2012}. %({\color{red} cite nugget paper})
...
%due to the limited range of the production function that is observed \cite{hilborn_quantitative_1992}. 
%This example is a "low contrast" setting.
%
%%
%Figure (\ref{flatTrio}) shows the biases in $F^*$ and $\frac{B^*}{\bar B(0)}$ 
...
%respectively, and the Ricker model is a \mbox{limiting case as $\gamma\rightarrow0$ \cite{schnute_general_1985}.}
%
%%
%\vspace{-0.5cm}
%\begin{figure}[h!]
...
%representation of simple age-structured dynamics \cite{deriso_harvesting_1980, schnute_general_1985, schnute_general_1987}. %\color{red} cite equivalnece}. 
...
%
%%
%The DD model is derived directly from an assumption of Von Bertalanffy growth \cite{von_bertalanffy_quantitative_1938} %{\color{red} cite } 
...
%%	\item Equilibrium calcutions are avilable \cite{schnute_analytical_1998}
%%\end{itemize}
%%
%%
%%then 
...
%%}
%%
%%\cite{deriso_harvesting_1980}
%%\cite{schnute_general_1985, schnute_general_1987}
%%\cite{walters_continuous_2020}
%%
%%
%%
%%
...
%%
%%
%%%\citeA{punt_extending_2019} formulated this way; mathematically equivalent 
%%%to formulations in \cite{schnute_general_1985, schnute_analytical_1998} which 
...
%%in the northeastern pacific \cite{narita_pacfin_1988,calcom_california_2006}. 
...
%%methods for quantifying catch uncertainty have been developed \cite{shelton_estimating_2012}. 
...
%%%assessment models \cite{methot_stock_2013}.
%%
%%%Improved methods of quantfying catch uncertainty exists \cite{shelton_estimating_2012}, 
...
%%%uncertainty have been developed \cite{shelton_estimating_2012}, the 
%%%effect of uncertainty in catch has not been incorporated in stock assessment 
%%%models \cite{methot_stock_2013}.    
%%
%%%
%%The theoretical use of splines for smoothly and accurately approximating the 
%%solution of initial value problems has been well established \cite{ahlberg_theory_1967}. %\cite{ahlberg_convergence_1963, ahlberg_cubic_1968, ahlberg_collocation_1975}.  
%%As a statistical model splines are also well established as a nonparametric 
%%(or semiparametric) regression technique \cite{green_nonparametric_1993, hastie_elements_2009}. %\cite{de_boor_practical_1978, wahba_spline_1990, green_nonparametric_1993, hastie_elements_2009}.
...
%\cite{walters_calculation_1994, deyoreo_integrating_2012} %({\color{red} Cite Maria DeYorio pdf??}).
%
%%
...
%similarly to \cite{schnute_analytical_1998} and (\ref{BratS}) 
...
%Matern correlation function \cite{matern_spatial_1960}. The updated correlation structure 
...
%mortality are a'priori fixed constants \cite{mangel_perspective_2013, maunder_is_2003}. 
...
%been studied \cite{punt_extending_2019}. 
%
%%
%The focus here is on three parameter SRRs which have the Beverton-Holt or 
%Schaefer models as a special case \cite{shepherd_family_1982, pella_generalized_1969}.
%%\cite{deriso_harvesting_1980, schnute_general_1985, shepherd_family_1982}. 
...
%\mbox{source may greatly influence stock status \cite{fr2011}.} 
...
%\citeA{fr2014, tj2017} the multinomial integrating model is often poorly 
...
%Namely \citeA{fr2014} proposes replacing the multinomial distribution with the 
%more flexible logit-normal \mbox{distribution and \citeA{tj2017} proposes the 
...
%are typically slower than local optimizers \cite{mp2013}.
%
...
%technique \cite{gl2015} that has seen a wide variety of applications, but
%of notable success in the realm of computer simulation experiments 
%\cite{jo1998, gr2016}. Bayesian optimization gets its name from the 
...
%models while accounting for model uncertainty \cite{oh1998} as well as to
%correct model inadequacy problems \cite{ko2001}.
...
%MSE \cite{ly2018}. MSEs balance trade-offs across multiple management 
...
%different stocks \cite{pu2014}, and they are widely acknowledged to be the 
...
%%\cite{pu2014}, and are widely acknowledged to be the most appropriate way to 
...
%be a function of subjective value systems \cite{fr2011}. Without fully 
...
%defined analytical properties \cite{gc2013}.
...
%\cite{dlm2018, ch2018} defines a set of utilities ($\bm{\Omega}$) associated 
