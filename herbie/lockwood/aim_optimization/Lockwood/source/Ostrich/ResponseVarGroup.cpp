...
******************************************************************************/
ResponseVarGroup::ResponseVarGroup(char * pToken)
{
   char * pStart;
   char * pEnd;
...
void ResponseVarGroup::Destroy(void)
{
   ...
   for(i = 0; i < m_NumRespVars; i++) { delete m_pRespVarList[i];}
   delete [] m_pRespVarList;

   for(i = 0; i < m_NumTiedRespVars; i++) 
   {
      m_pTiedRespVarList[i]->Destroy(); 
...
{
   char * start_tag = "BeginResponseVars";
   char * end_tag   = "EndResponseVars";
   ...
   {      
      ...
      m_pRespVarList[i] = new ResponseVar(tmpName,tmpFile,tmpKey,line,col,tok);
...
******************************************************************************/
void ResponseVarGroup::InitFromFile(IroncladString respFileName, char * start_tag, char * end_tag)
{
   int    start_len = (int)strlen(start_tag);
   int    end_len   = (int)strlen(end_tag);
...
{
   char * start_tag = "BeginTiedRespVars";
   char * end_tag   = "EndTiedRespVars";
   int    start_len = (int)strlen(start_tag);
   int    end_len   = (int)strlen(end_tag);
