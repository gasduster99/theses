...
02-16-04    lsm   added algorithms from Numerical Recipes
...
//(0.5 * (3.00 - sqrt(5.00))) 
#define CGLD  (0.38196601125010515179541316563436)
//(2.0 - (0.5 * (3.00 - sqrt(5.00))))
#define GOLD  (1.6180339887498948482045868343656)
#define TINY (1.0e-20)
#define GLIMIT (100.0)
#define ITMAX  (36)
#define ZEPS (NEARLY_ZERO)
...
Brackets the minimum using the algorithm presented in Numerical Recipes in C,
2nd edition, Press, et. al., page 400-401. 
      ...
      while(fb > fc)
      {
         ...
         else if((c - u)*(u - ulim) > 0.0)
         {
            ...
            if(fu < fc)
            {
               ...
               SHFT(fb, fc, fu, CalcF(u, fmin, xmin))    
               m_BoundMinCount++;
            }/* end if() */
         }/* end else if() */
...
{   
   ...
   ---------------------------------------------------------------------*/
   its = (int)((log10(m_SearchConvVal/fabs(xL-xU))/log10(1.00-CGLD)) + 1.00);

   Write1dSearch(WRITE_GSECT, its+2);
   x1 = ((1.00 - CGLD) * xL) + (CGLD * xU);
...
Uses Brent's method, as presented in Numerical Recipes in C, 2nd edition, 
Press, et. al., page 404-405. 
...
MinPtStruct * OptSearchClass::Brent(MinBracketStruct * pBrack, double * minf, double * minp)
{
   ...
   Write1dSearch(WRITE_BRENT, ITMAX);
   for(iter = 1; iter <= ITMAX; iter++)
   {
      ...
      if(fabs(d) >= tol1){ u = x + d;} else{u = x + SIGN(tol1,d);}
