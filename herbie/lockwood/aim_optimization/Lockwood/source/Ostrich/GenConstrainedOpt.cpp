...
{
   char * start_tag = "BeginGCOP";
   char * end_tag   = "EndGCOP";
   ...
   m_pCostFunc = m_pRespGroup->GetRespVarPtr(costStr);
   if(m_pCostFunc == NULL)
   {
...
{
   if(m_pConstraints != NULL){ m_pConstraints->Destroy();} 
   delete m_pRespGroup;
...
double GCOP::CalcObjFunc(void)
{
   ...
   if(penalty != 0.00)
   {
      switch(m_PenType)
      {
         ...
         case(PEN_TYPE_MPM) :
         {
            cost = FMAX(cost, penalty) * (1.00 + penalty);
            break;
         }         
         case(PEN_TYPE_EPM) :
         {
            if(penalty >= NEARLY_HUGE_LN_EXP){ cost = NEARLY_HUGE; break;}            
            cost = FMAX(cost, penalty) * exp(penalty);
            break;
         }         
      }/* end switch() */
   }/* end if() */
