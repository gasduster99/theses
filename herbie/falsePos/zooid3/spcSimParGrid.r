...
#
rosenbrock = function(x, p=2){
        #
	x = matrix(x, ncol=p, byrow=T)
	#xi = x[,1:(p-1)]
	#xip1 = x[,2:p]
	#out = rowSums(100*(xi^2 - xip1)^2 + (xi - 1)^2)
	#
	out = matrix(0, nrow=dim(x)[1], ncol=1)
	for(i in 1:(p-1)){
...
#
gridOpt = function(f, grid=seq(0.1, 0.9, 0.1)){
        #
        l = c()
        for(g in grid){ l=c(l,f(g)) }
        lMin = min(l)
        gMin = grid[l==lMin]
        #
        return( list(value=lMin, argMin=gMin) )
}
...
#wGrid = seq(20, 100, 5) #seq(20, 80, 5)
#itMax = 300
##
	...
	while( flags$inLoop ){
		...
		for(w in wGrid){
			...
			if( it>w ){
				...
				optOut = gridOpt( function(x){theFunk(x, Zmax, samples, sLen, ewmaConvChart, w, slice, LNToN, getVect, ewma, ssError)} )
                                lambda = median(optOut$argMin, na.rm=T)
