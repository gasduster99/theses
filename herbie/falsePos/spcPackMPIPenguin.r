...
output = function(isOut, name, Zmax, it, mmu, lambda, ewma, W, test1){
	...
	if(isOut){
		...
		cmdString = sprintf("pdflatex '\\def\\bestName{bestZ%s.pdf} \\def\\ewmaName{ewmaConvChart%s.pdf} \\input{seeOut.tex}'", name, name)
        	system( cmdString )
		system( sprintf('mv bestZ%s.pdf /home/nickgrunloh/Documents/falsePos/pictures/', name) )
		system( sprintf('mv ewmaConvChart%s.pdf /home/nickgrunloh/Documents/falsePos/pictures/', name) )
		system( sprintf('cp seeOut.pdf /home/nickgrunloh/Documents/falsePos/pictures/seeOut%s.pdf', name) )
	}
}
...
clusterOut = mpi.applyLB( 1:M, 
	...
	dir.create(sprintf('/home/nickgrunloh/Documents/falsePos/rank%s', rank))
	#/home/nick/Documents/falsePos
	setwd(sprintf('/home/nickgrunloh/Documents/falsePos/rank%s', rank))
...
#
save.image(sprintf('%sSimW%d.RData', name, W))
}
