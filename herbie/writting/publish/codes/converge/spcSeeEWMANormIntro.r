...
name = "RastHard"; uu=50; tMin=0; lambda=0.4
...
wasd = function(name){
	cmdString = sprintf("pdflatex '\\def\\ewmaName{introChart%s.pdf} \\input{seeOutIntro.tex}'", name, name)
...
while(T){
	...
	max2 = 0 + 1 #max(mmu, ewmaOut$y, ewmaOut$limits)
	#
	#Figure 2
	#dev.new()
	...
	plot( seq(1, it), rev(c(ewmaOut$statistics, ewmaOut$newstats)),
		...
		xlab="Iteration",
		ylab="ELAI",
		main="Rastrigin Test Function" #"Rosenbrock Test Function" #
	)
