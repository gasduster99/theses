...
void NetCDFConverter::ReadResponse(void)
{
   if (strncmp(m_accessType, "Read", 4) == 0)
   {
      ...
      s_fileName = s_fileName.substr(0, s_fileName.find_last_of('.'));
      s_fileName += ".txt";
	   m_command = "nc2text " + string(m_fileName) + " " + string(m_arrayName) + "[" + string(m_itemPos) + "]";
		ExecuteCommandLine(m_command.c_str(), true, s_fileName.c_str(), m_name);
   } 
}/* end ReadResponses() */
...
bool NetCDFConverter::WriteParameter(char * pName, char * pValue)
{
   if((strcmp(pName, m_param) == 0) && (strcmp(m_accessType, "Write") == 0))
   {
      ...
      s_fileName = s_fileName.substr(0, s_fileName.find_last_of('.'));
      s_fileName += ".txt";
	   m_command = "echo " + string(pValue) + " | text2nc "+ string(m_fileName) + " " + string(m_arrayName) + "[" + string(m_itemPos) + "]";
		ExecuteCommandLine(m_command.c_str(), false, s_fileName.c_str(), m_name);
      return true;
	}
   else
   {
