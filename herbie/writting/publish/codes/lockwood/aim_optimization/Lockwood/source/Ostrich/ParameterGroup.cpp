...
void ParameterGroup::ConfigureSpecialParams(double minObj, double minCon)
{
   ...
   {
      m_pSpecial[j]->SetEstVal(minObj, minCon);
   }/* end for() */
} /* end ConfigureSpecialParams() */

/******************************************************************************
GetSpecialConstraint()

Retrieve the current value of the response variable associated with the
current constraint.
******************************************************************************/
double ParameterGroup::GetSpecialConstraint(void)
{
   ConstraintABC * pCon;
   double rv = 0.00;
...
void ParameterGroup::InitRealParams(IroncladString pFileName)
{
   ...
   {            
      ...
      sscanf(line, "%s %s %lf %lf %s %s %s", tmpName, tmpInitVal, &lowerBound,
             &upperBound, tmpTrans1, tmpTrans2, tmpTrans3);
...
void ParameterGroup::InitTiedParams(IroncladString pFileName)
{
   ...
   {      
	  ...
	  if(np != 0)
	  {
		...
		NEW_PRINT("ParameterABC *", np);
		pParams = new ParameterABC * [np];
		for(n = 0; n < np; n++)
		{
		   j = ExtractString(pTok, tmpStr);
		   j = ValidateExtraction(j, n, np, "InitTiedParams()");
           pTok += j;
           pParams[n] = GetParamPtr(tmpStr);
           if(pParams[n] == NULL)
           {
         ...
         { 
            ...
            { 
               ...
               m_pTied[i] = new TiedDistXY(nameStr, pParams[0], pParams[1], pParams[2], pParams[3]);
            }
            else{ invalidNumParams = true;}
         }
         else
