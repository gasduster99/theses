...
void LevenbergAlgorithm::CalibrateGML(void)
{
   ...
   //fill in vectors of computed observations and residuals
   ...
   while(done == 0)
   {
      ...
      if(id == 0)
      {
         ...
         //write iteration data
         WriteRecord(m_pModel, iter, m_Phi, m_Lambda);
         pStatus.curIter = iter;
         pStatus.pct = (((float)100.00*(float)GMLcount)/(float)m_NumMS) + 
                       (((float)100.00*(float)iter)/((float)m_MaxIter*(float)m_NumMS));
         pStatus.numRuns = m_pModel->GetCounter();
         WriteStatus(&pStatus);
...
{
   m_pWeights = m_pStats->GetWeights();
   m_pJacob   = m_pStats->CalcJacobian();
   m_pJacobT  = m_pStats->GetJacobT();
   m_pStats->AdjustJacobian();
   m_pStats->AdjustWeights();
}/* end CalcJacobian() */
...
void LevenbergAlgorithm::CalcUpgrade(void)
{
   ...
   //multiply result by Q [oXo], result is [pxo]
   MatMult(m_pPbyO1, m_pWeights, m_pPbyO2, p, n, n);
   //multiply result by J [oxp], results is [pxp]
   MatMult(m_pPbyO2, m_pJacob, m_pPbyP1, p, n, p);
   ...
   //multiply result by Q [oXo], result is [pXo]
   MatMult(m_pPbyO2, m_pWeights, m_pPbyO1, p, n, n);
   //multiply result by residual [oX1], result is [pX1]
   VectMult(m_pPbyO1, m_pResid, m_pTmpVec, p, n);
...
void LevenbergAlgorithm::CalcGamma(void)
{
   ...
   VectMult(m_pJacob, m_pUpgrade, m_pGamma, n, p);
}/* end CalcGamma() */
...
void LevenbergAlgorithm::CalcBeta(void)
{
   ...
   {
      /*-----------------------------------------------------
      NOTE: m_pWeights corresponds to Q in WinPEST Manaul,
      therefore, m_pWeights[i][i] = wi*wi in equation 2.24
      ------------------------------------------------------*/
      numer += (m_pResid[i]*m_pWeights[i][i]*m_pGamma[i]);
      denom += (m_pWeights[i][i]*m_pGamma[i]*m_pGamma[i]);
   }/* end for() */
...
******************************************************************************/
LevenbergAlgorithm::~LevenbergAlgorithm(void)
{  
   Destroy();
}/* end DTOR */
