...
typedef struct PSO_PARTICLE_STRUCT
{
	...
	double cx;  //constraint value (note - hacked in, limits pre-emption to a single constaint)
	double fb;  //local best obj. func.
	double cb;  //local best constraint (note - hacked in, limits pre-emption to a single constaint)
