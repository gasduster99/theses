...
typedef struct DURBIN_WATSON_STRUCT
{
   double D, lwr, upr;
   bool bSuccess;
}DurbinWatsonStruct;
...
class StatsClass
{
   public:
      ...
      Unchangeable2DArray GetWeights(void){ return m_pWeights;}
      void AdjustWeights(void);
      Unchangeable2DArray CalcJacobian(void);
      ...
      ~StatsClass(void){ Destroy();}           
      void Destroy(void);

   private:
      Unchangeable2DArray CalcJacobian(bool bOkToHoldParams, bool bOkToHoldObs);
      void BcastMinJac(void);
      void BcastJacobian(void);
      void EvalJacSerial(void);
      ...
      bool m_DurbinWatsonFlag; //default = false
      ...
      double ** m_pWeights; //matrix of observation weights (Q)
      ...
      /*The determinant of the correlation coefficient matrix. This term is 
      also the ratio of the joint confidence ellipsoid and confidence interval 
      block volumes, when the joint confidence ellipsoid is computed using 
      the m_EllipsePct value. */
      double    m_VolRatio; 
      ...
      uncorrelated (i.e. m_VolRatio = 1.00). */
      double    m_EllipsePct; 
   
      RunsStruct m_Runs;
      DurbinWatsonStruct m_DW;
      MMRI_Struct m_MMRI;
