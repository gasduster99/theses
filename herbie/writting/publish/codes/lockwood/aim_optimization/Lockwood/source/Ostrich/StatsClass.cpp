...
double StatsClass::AdjustObjFunc(double val)
{
   double tmp;
   int i;
   //restore 'true' weights and residuals
   AssignWeights();
   CalcResiduals();
   ...
   {      
      ...
      {
         tmp = m_pWeights[i][i] * m_pResid[i] * m_pResid[i];
         val -= tmp;
      }            
   }/* end for() */

   //restore adjusted weights and residuals
   AdjustWeights();
   AdjustResiduals();   
...
Adjust the weight matrix, if needed, to eliminate insensitive observations.
******************************************************************************/
void StatsClass::AdjustWeights(void)
{
   int i, j, k;
   ...
   if(m_NumHeldObs > 0)
   {
      ...
      for(j = 0; j < m_NumObs; j++)
      {
         ...
         else
         {
            ...
            {
               m_pWeights[k][k] = m_pWeights[k+1][k+1];
            }
            m_pWeights[k][k] = 0.00;
         }/* end else() */
      }/* end for() */
...
Assign the weights of the weight matrix.
******************************************************************************/
void StatsClass::AssignWeights(void)
{
   double weight;
   int i, j;
   Observation * pObs;
   ...
   NOTE: m_pWeights corresponds to the Q matrix in the WinPEST 
   manual, defined as the square of the observation weights, 
   since these are referenced to the standard deviation rather 
   than the variance.
   ----------------------------------------------------------*/
   for(i = 0; i < m_NumObs; i++)
   {
      ...
      for(j = 0; j < m_NumObs; j++)
      {
         if(i == j){ m_pWeights[i][j] = (weight * weight);}
         else { m_pWeights[i][j] = 0.00; }
      }/* end for() */
   }/* end for() */
}/* end AssignWeights() */
...
StatsClass::StatsClass(ModelABC * pModel)
{
   ...
   NEW_PRINT("double *", m_NumObs);
   m_pWeights = new double*[m_NumObs];
   ...
   {  
      NEW_PRINT("double", m_NumObs);
      m_pWeights[i] = new double[m_NumObs];
   }
   for(i = 0; i < m_NumObs; i++)
   {
      NEW_PRINT("double", m_NumObs);
   ...
   if(CheckToken(pFile, "BeginMathAndStats", pStatsFileName) == true)
   {
      ...
      */
      ...
      while(strstr(line, "EndMathAndStats") == NULL)
      {
         ...
         else if(strstr(line, "Durbin-Watson") != NULL){m_DurbinWatsonFlag = true;}
...
void StatsClass::Destroy(void)
{
   ...
   for(i = 0; i < m_NumObs; i++){delete [] m_pWeights[i];}
   delete [] m_pWeights;
...
{   
   ...
   {
      pObs = pObsGroup->GetObsPtr(i);
      calcObs = pObs->GetComputedVal();
      measObs = pObs->GetMeasuredVal();
      m_pResid[i] = (measObs - calcObs);
   }/* end for() */
...
******************************************************************************/
Unchangeable2DArray StatsClass::CalcJacobian(void)
{   
   return (CalcJacobian(true, true));
}/* end CalcJacobian();
...
******************************************************************************/
Unchangeable2DArray StatsClass::CalcJacobian(bool bOkToHoldParams, bool bOkToHoldObs)
{   
   int n, i, id, j;
   char msg[DEF_STR_SZ];
   FILE * pFile;
...
retry:
         ...
         for(i = 0; i < m_NumObs; i++)
         {
            midObs = m_pMidBkup->GetObs(i);
            hiObs = m_pHiBkup->GetObs(i);

            if(dType != FD_FORWARD)
            {
               lowObs = m_pLowBkup->GetObs(i);
            }/* end if() */
         ...
         if((total_diff <= NEARLY_ZERO) && (dIncType != FD_RANGE_REL))
         {
   ...
   for(proc = 0; proc < num_procs; proc++)
   {
      //receive someones flat Joacobian, this will clobber myBuf
      MPI_Bcast(m_pMyBuf, bufsize, MPI_DOUBLE, proc, MPI_COMM_WORLD);

      //store the other processors results into the Jacobian matrix
      for(j = proc; j < m_NumParams; j += num_procs)
      {
         for(i = 0; i < m_NumObs; i++)
         {
            idx = (i*m_NumParams) + j;
            m_pJacob[i][j] = m_pMyBuf[idx];
            m_pJacobT[j][i] = m_pMyBuf[idx];         
         }
      }/* end for(store) */
...
******************************************************************************/
void StatsClass::EvalJacSerial(void)
{
   bool flipSign;
   double upr;    //upper bound of parameter
   double lwr;    //lower bound of parameter
...
Unchangeable2DArray StatsClass::CalcNormal(void)
{
   ...
   //compute (transpose of Jacobian [pXo]) * (weight matrix [oXo])
   MatMult(m_pJacobT, m_pWeights, m_pPbyO1, p, n, n);

   //compute pTemp [pXo] * J [oXp] = normal matrix [pXp]  
   MatMult(m_pPbyO1, m_pJacob, m_pNormal, p, n, p);
...
void StatsClass::CalcStats(void)
{
   ...
   CalcJacobian(m_bOkToHoldParams, m_bOkToHoldObs); //compute Jacobian, possibly in parallel
   ...
   if(id == 0)
   {
      ...
      if(m_bInv == false)
      {
         ...
         LogError(ERR_CONTINUE, "  (7)  linearity measures,");
         LogError(ERR_CONTINUE, "  (8)  KIC MMRI measure, and ");
         LogError(ERR_CONTINUE, "  (9)  Durbin-Watson test.");
         LogError(ERR_CONTINUE, "  (10) Prediction statistics.");
      ...
      if(m_RunsTestFlag == true){
         m_Runs.bSuccess = RunsTest(m_pResid, n, &(m_Runs.pos), 
                                    ...
                                    &(m_Runs.lwr), &(m_Runs.upr)); }      

      if(m_MMRI_Flag == true) CalcMMRI(m_bInv);

      if(m_DurbinWatsonFlag == true){ 
         m_DW.bSuccess = DurbinWatsonTest(m_pResid, m_pJacob, n, p, 
                                          &(m_DW.D), &(m_DW.lwr), &(m_DW.upr)); }
...
void StatsClass::CalcCI(void)
{
   ...
   /*----------------------------------------------------------------------
   Calculate the volume ratio statistic.

   This is suggested by Draper & Smith to be a useful indicator of whether 
   or not the rectangular CI block (which is defined by the combination of 
   inidividual linear CI's) can be used to approximate the ellipsoidal 
   joint confidence region for all parameters.
   -----------------------------------------------------------------------*/
   m_VolRatio = sqrt(CalcDeterminant(coeff, params));
...
void StatsClass::CalcBealeAndLinssen(void)
{
   ...
   */
   ...
   for(j = 0; j < numSets; j++)
   {
      ...
      -----------------------------------------------------------*/
      MatMult(pNumerT, m_pWeights, pTemp, 1, n, n);
      VectMult(pTemp, pNumer, pResult, 1, n);
      numerSum += pResult[0];

      MatMult(pDenomBT, m_pWeights, pTemp, 1, n, n);
      VectMult(pTemp, pDenomB, pResult, 1, n);
      denomSumB += (pResult[0] * pResult[0]);

      MatMult(pDenomLT, m_pWeights, pTemp, 1, n, n);
      VectMult(pTemp, pDenomL, pResult, 1, n);
...
observations in on-linear regression modeling of groundwater flow", Water 
...
{
   double ** sqrtW; //square root of the weight matrix (diag. elements only)
   double ** wJ; //sqrtW * Jacobian
   double ** wJT; //transpose of (sqrtW * Jacobian)
   ...
   for(i = 0; i < n; i++)
   {
      for(j = 0; j < n; j++)
      {
         if(i == j){ sqrtW[i][j] = sqrt(m_pWeights[i][j]);}
         else { sqrtW[i][j] = 0.00;}
      } /* end for() */
   }/* end for() */

   /*-------------------------------------------------------------
   Allocate and assign the wJ and wJT matrices
   -------------------------------------------------------------*/
   MatMult(sqrtW, m_pJacob, wJ, n, n, p);

   for(i = 0; i < p; i++)
   {
      for(j = 0; j < n; j++) { wJT[i][j] = wJ[j][i];}
   }/* end for() */

   /*-------------------------------------------------------------
   Assign the intermediate, Hat and Change matrices
   -------------------------------------------------------------*/
   MatMult(wJ, m_pInvNormal, pTmp, n, p, p);
   MatMult(pTmp, wJT, m_pHat, n, p, n);
   MatMult(m_pInvNormal, wJT, m_pChange, p, p, n);   
   
   /*-------------------------------------------------------------
   Free up matrices
   -------------------------------------------------------------*/
   for(i = 0; i < n; i++){delete [] sqrtW[i];}
   delete [] sqrtW;
   for(i = 0; i < n; i++){delete [] wJ[i];}
   delete [] wJ;
   for(i = 0; i < p; i++){delete [] wJT[i];}
   delete [] wJT;
...
void StatsClass::CalcCooksD(void)
{
   ...
   {      
      ...
      m_CooksAvgLvg += Hii;
      //ri = (ei * sqrt(Qii)) / (sqrt(ss) * sqrt(1 - Hii));      
   }/* end for() */      
   m_CooksAvgLvg /= n;
...
void StatsClass::CalcDFBETAS(void)
{
   ...
   {
      Hii = m_pHat[i][i];
      fi  = (sqrt(m_pWeights[i][i]) * m_pResid[i]);
      ssi = (1.00/(n - p - 1.00));
...
void StatsClass::CalcNormProbPlot(void)
{
   ...
   double ui, numer, denom1, denom2, ei, m, pi;
   ...
   for(i = 0; i < n; i++)
   {      
      m_pOrdResid[i] = m_pResid[i]*sqrt(m_pWeights[i][i]);
...
void StatsClass::CalcWeightedRy(void)
{
   ...
   for(i = 0; i < m_NumObs; i++)
   {
      if(m_bHoldObs[i] == false){
         pObs = pObsGroup->GetObsPtr(i);
         wgt = sqrt(m_pWeights[j][j]);
         avgObs += (wgt*pObs->GetMeasuredVal());
         avgEst += (wgt*pObs->GetComputedVal());
         j++; }
   }/* end for() */
   ...
   {  
      if(m_bHoldObs[i] == false){    
         pObs = pObsGroup->GetObsPtr(i);
         wgt = sqrt(m_pWeights[j][j]);
         Yhat = (wgt*pObs->GetComputedVal());
         Ytrue =(wgt*pObs->GetMeasuredVal());
...
void StatsClass::CalcRawRy(void)
{
   ...
   for(i = 0; i < m_NumObs; i++)
   {
      if(m_bHoldObs[i] == false){
         pObs = pObsGroup->GetObsPtr(i);
         avgObs += (pObs->GetMeasuredVal());
         avgEst += (pObs->GetComputedVal());
         j++; }
   }/* end for() */
   ...
   {  
      if(m_bHoldObs[i] == false){    
         pObs = pObsGroup->GetObsPtr(i);
         Yhat = (pObs->GetComputedVal());
         Ytrue =(pObs->GetMeasuredVal());
         numer += ((Ytrue-avgObs)*(Yhat-avgEst));
         tmp1 += ((Ytrue-avgObs)*(Ytrue-avgObs));
         tmp2 += ((Yhat-avgEst)*(Yhat-avgEst));
         j++; }
...
invertible. If not invertible, KIC measure will not be computed.
******************************************************************************/
void StatsClass::CalcMMRI(bool bInv)
{
   ...
   if(bInv == true) //only compute KIC if Normal matrix is invertible
   {
      det = CalcDeterminant(m_pNormal, k-1);
      m_MMRI.KIC  = (n * log(ss)) + (k * log(n/(2.00 * MY_PI))) + log(det);
   }
}/* enc CalcMMRI() */
...
void StatsClass::CalcSensitivities(void)
{
   ...
   double wt;   //sqare root of the weight
   double ss;
...
void StatsClass::WriteStats(FILE * pFile)
{
   ...
   double m, p, d; //measured, predicted, difference
   ...
   fprintf(pFile, "Observation    Measured       Simulated      Weighted Residual");
   ...
   {
      pObs = pObsGroup->GetObsPtr(i);
      m = pObs->GetMeasuredVal();
      p = pObs->GetComputedVal();
      d = sqrt(m_pWeights[i][i])*(m-p);
      fprintf(pFile, "%-13s  %E  %E  %+E   ", pObs->GetName(), m, p, d);
      if(m_bOkToHoldObs == true){
   ...
   fprintf(pFile, "\nCorrelation between raw measured and simulated observations (no weighting)\n");
   fprintf(pFile, "Ry         : %6.3lf\n", m_RawRy);
   fprintf(pFile, "Ry-squared : %6.3lf\n", m_RawRy*m_RawRy);

   fprintf(pFile, "\nCorrelation between weighted measured and weighted simulated observations\n");
   ...
   if(m_RunsTestFlag == true)
   {
      ...
      if(m_Runs.bSuccess == true)
      {
         ...
         fprintf(pFile, "Lower-tail p-value : %lf\n", m_Runs.lwr);
         fprintf(pFile, "Upper-tail p-value : %lf\n\n", m_Runs.upr);
         fprintf(pFile, "Residuals of zero are counted as positive\n");
         fprintf(pFile, "H0 : runs are randomly distributed\n");
         fprintf(pFile, "Ha : runs are non-random (i.e. clustered)\n");
      }
   ...
   if(m_DurbinWatsonFlag == true)
   {
      fprintf(pFile, "\nDurbin-Watson Test for 1st Order Autocorrelation of Residuals\n");
      if(m_DW.bSuccess == true)
      {
         fprintf(pFile, "Test Statistic (D) : %lf\n", m_DW.D);
         fprintf(pFile, "Lower-tail p-value : %lf\n", m_DW.lwr);
         fprintf(pFile, "Upper-tail p-value : %lf\n\n", m_DW.upr);
         fprintf(pFile, "D ~ 0 --> positive auto-correlation\n");
         fprintf(pFile, "D ~ 2 --> no auto-correlation\n");
         fprintf(pFile, "D ~ 4 --> negative auto-correlation\n");
         fprintf(pFile, "H0 : no auto-correlation\n");
         fprintf(pFile, "Ha (lower tail) : positive auto-correlation\n");
         fprintf(pFile, "Ha (upper tail) : negative auto-correlation\n");
      }
      else fprintf(pFile, "The Durbin-Watson Test was unsuccessful\n");
   }
   ...
   if(m_MMRI_Flag == true)
   {
      ...
      if(m_bInv == true)
      {
         fprintf(pFile, "Kashyap's Information Criterion        (KIC)  : %02lf\n", m_MMRI.KIC);
      }
      else
      {
         fprintf(pFile, "Kashyap's Information Criterion        (KIC)  : not computed\n");
      }
   }
   ...
   if(m_CIflag == true)
   {
      ...
      fprintf(pFile,"Volume Ratio : %6.3lf\n", m_VolRatio);
   }/* end if() */
   ...
   if(m_CooksFlag == true)
   {
      ...
      for(i = 0; i < m_NumObs; i++)   
      {
         pObs = pObsGroup->GetObsPtr(i);
         if(m_bHoldObs[i] == false) {
            fprintf(pFile, "%-13s  %.2E  ", pObs->GetName(), m_pHat[ii][ii]);
   ...
   if(m_DfbetasFlag == true)
   {
      ...
      for(i = 0; i < m_NumObs; i++)
      {
         ...
         if(m_bHoldObs[i] == false) {
            fprintf(pFile, "%-13s  ", pObs->GetName());
      ...
      for(i = 0; i < m_NumObs; i++)
      {
         ...
         for(j = 0; j < m_NumParams; j++)
         {
            if((m_bHoldObs[i] == false) && (m_bHoldParam[j] == false)){
               fprintf(pFile, "%+E ", m_pJacob[ii][jj]);}
            else{
...
{  
   ...
   *num_left = *num_left - samples_per_iter;
   ...
   for(i = 0; i < np; i++)
   {
      ...
      MPI_Bcast((void *)F, np, MPI_DOUBLE, i, MPI_COMM_WORLD);
      pList[i+j][num] = F[i];
   }/* end for() */
