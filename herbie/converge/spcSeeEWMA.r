...
while(T){
	...
	lines( seq(ig1, thresh), UCLN[ig1:thresh], col="blue", lty=2 )	
	lines(  seq(thresh, it),  UCLN[thresh:it], col="blue" )
	legend("topright",
		legend = c( sprintf("%1.1f Sigma log-N Interval", sig), sprintf("%1.1f Sigma N Interval", sig)),
		lty = c(1, 1), 
		col = c("red", "blue")
	)
	#
