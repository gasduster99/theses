...
wasd = function(name){
	...
	if( move=="d" | move=="w" ){
		...
		saveBest = sprintf("cp ./bestZ%s.pdf /home/nick/Documents/school/ucscGrad/thesis/herbie/writting/figures/bestZ%s%s.pdf", name, name, add)
		saveEWMA = sprintf("cp ./ewmaConvChart%s.pdf /home/nick/Documents/school/ucscGrad/thesis/herbie/writting/figures/ewmaConvChart%s%s.pdf", name, name, add)
		system( saveBest )
		system( saveEWMA )
	}else{
...
while(T){
	...
	wmu = wOut[[2]][1]
	ws2 = wOut[[2]][2]
	#a=( wmu^2*(1-wmu)-ws2*wmu )/ws2; b=(a/wmu) - a; fudge=rbeta(1, a, b)
	#fudge=rnorm(1, wmu, ws2/sqrt(mcmcSize))
	#
	...
	#rnorm(1, min(mm), min(ms2)/sqrt(mcmcSize))
	#
	#Figure 1
