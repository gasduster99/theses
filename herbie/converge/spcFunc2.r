...
rect = cbind(c(-1,-1), c(1, 1))#cbind(c(-2, -2), c(2, 2))#

#f= rosenbrock
#rect = cbind(c(0.9, -4), c(4, 0.9))
...
for( i in seq(1, M) ){
	...
	#
	samLower = quantile(wSamples, probs=0.0013)
	samUpper = quantile(wSamples, probs=0.9987)
	#
	#OUTPUT
	ding(1)
	...
	plot( seq(1, i), maxes[1:i], 
		...
		ylim=c(0, max(wupper3, maxes[1:i], samUpper))
	)	
        left = max((L-(W-1)), 1)
	#Sample Intervals
	segments(    1, samUpper, left, samUpper, col="blue", lty=2)
	segments(    1, samLower, left, samLower, col="blue", lty=2)
	segments( left, samUpper,    i, samUpper, col="blue")
	segments( left, samLower,    i, samLower, col="blue")	
